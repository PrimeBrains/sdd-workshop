...
コードには WHAT は残るが WHY は残らない。AI が書いたコードはその傾向がさらに深刻で、説明はチャット履歴に消える。SDD が提供するのは WHY を書き残す**場所**だ——`design.md` にトレードオフや判断の根拠を記述し、ステアリング文書にプロジェクト横断の文脈を蓄積することで WHY が永続する。ただし、場所があっても意識して書かなければ残らない点に注意が必要だ。ADR（Architecture Decision Record）のような追加実践と組み合わせることでさらに強化できるが、これは Kiro の標準機能ではなくオプションだ。
