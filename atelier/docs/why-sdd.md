...
**EARS（Easy Approach to Requirements Syntax）** とは `WHEN {条件} THE SYSTEM SHALL {振る舞い}` の構文で要件を記述する形式だ。公式ドキュメントは EARS を _"unambiguous and easy to understand"_ かつ _"directly translatable into test cases"_ と位置づけている。主体・条件・動作を明示することで AI の解釈ブレを防ぐが、EARS の真の価値は「誤解を消す構文」ではなく「誤解を可視化して検証しやすくする構文制約」にある。
...
コードには WHAT は残るが WHY は残らない。AI が書いたコードはその傾向がさらに深刻で、判断の根拠はチャット履歴に残るだけだ。仕様がなければ「なぜこの設計にしたか」「なぜこのトレードオフを選んだか」はセッション終了とともに消える。

`design.md` はそのための場所だ。SDD では実装の前に設計判断とトレードオフを `design.md` に書き切ることが求められるため、WHY を残す行為がワークフローに組み込まれている。仕様ファイルは git 管理されるので、半年後のチームメンバーも AI エージェントも同じ文脈から作業を始められる。ステアリング文書はこれをプロジェクト横断のレベルで補完する。

なお、ADR（Architecture Decision Record）のような追加実践と組み合わせることでさらに強化できるが、これは Kiro の標準機能ではなくオプションだ。
...
| **フィーチャーレイヤー**             | **← SDD が有効**                              |
| 実装品質・テスト品質・依存管理・運用 | それぞれ専用の実践が必要                      |
...
4つの観点はいずれも義務的に実施するものだが、「決定論的に自動化できるか否か」で対処手段が変わる。**advisory** とは「強制的にブロックするゲートではなく、判断を促す勧告」のことで、自動化できないチェックを人間のワークフローに組み込む手段だ。
...
```mermaid
graph TD
    L1["🔒 層1: 存在チェック<br/>スクリプト（決定論的）"]
    L2["⚡ 層2: EARS → PBT<br/>（準決定論的）"]
    L3["💬 層3: LLM advisory<br/>（非決定論的）"]
    L1 --> L2 --> L3
...
| 層  | 内容                                  | 決定論性     | 運用                                     |
| --- | ------------------------------------- | ------------ | ---------------------------------------- |
| 層1 | スペックファイル存在チェック          | 完全決定論的 | 強制ゲート化可                           |
| 層2 | EARS → Property-based Testing         | 準決定論的   | 構文解析は確定、property化に解釈が入る   |
| 層3 | LLM advisory（`/kiro-validate-impl`） | 非決定論的   | 承認ゲート組み込みで実効カバレッジを確保 |
...
| 戦略                     | 意味                                                    | 説明                     | 向いているケース         |
| ------------------------ | ------------------------------------------------------- | ------------------------ | ------------------------ |
| **ボーイスカウット**     | 触った箇所は来たときより良くして帰る、というルール      | 触った箇所から仕様を書く | 高頻度改修箇所がある     |
| **ストラングラーフィグ** | 古い木を新しい木が包んで置き換えていく植物に由来するパターン | 新機能は SDD 必須にする  | 新機能開発が続く         |
