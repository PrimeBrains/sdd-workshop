...
test('シナリオ 7: ProgressInputPanel で進捗率変更 → 保存 → モーダル背景の Gantt 行の進捗バーが更新される', async ({
  ...
  // 連続実行で同じ値が DB に残ると「保存」ボタンが disabled になるため、
  // 現在値に依らず必ず変化させる（>=90 なら -5、それ未満は +5）
  ...
  // 入力値が確実に変わったことを input から直接読む（React 再描画タイミングに依存しない）
  await expect(progressNumber).toHaveValue(target)
