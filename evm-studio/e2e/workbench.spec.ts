...
/** SummaryStat 「SPI」の value テキスト（label 直下の値） */
async function readSummaryStat(page: Page, label: 'SPI' | 'CPI' | 'EV' | 'PV' | 'AC' | 'VAC' | 'BAC'): Promise<string> {
  // 各 SummaryStat は <div>{label}</div><div>{value}</div> の構造
...
/** GanttChart 内の任意の行（テキスト一致で 1 件目を取得） */
function ganttRowByText(scope: Locator | Page, text: string): Locator {
  const root = 'locator' in scope ? scope : scope.locator('body')
  // モックアップ準拠の GanttChart 行は code + name を含む 1 つの <div>
  return root.locator('div').filter({ hasText: text }).first()
}
...
test('シナリオ 8: SPI トレンドの「全画面で見る」クリック → ChartFullscreen 表示、背景クリックで閉じる', async ({
  ...
  // SPI Trend モーダルが開く
  const trendModalHeading = page.getByText('SPI / CPI 推移').last()
  await expect(trendModalHeading).toBeVisible({ timeout: 5_000 })

  // 背景クリック (modal overlay は inset=0, z=9999 の最上位 div)。
  // モーダル外の左上 (5, 5) を強制クリックすると onClick(overlay) → onClose が走る
  await page.mouse.click(5, 5)

  // 閉じたことを確認（トレンドの大見出しが消える）
  await expect(page.getByText('SPI / CPI 推移')).toHaveCount(0, { timeout: 5_000 })
})
