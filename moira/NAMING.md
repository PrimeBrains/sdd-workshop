...
> **追記（2026-07-05・未ゲート・D-73/issue #23）:** 提示層の語「**ポートフォリオ / portfolio**」＝複数プロジェクト（home）の読み取り専用の**並置**（`moira ui --portfolio`）。**MODEL 外の概念**（本モデルの単一プロジェクト視点は不変 — D-50・MODEL §5）ゆえ上表の MODEL 従属語彙には含めない。「束ねる」は並置の意であり、会計の**合成**（横断 EV%/SPI/CPI のロールアップ）を意味しない。
