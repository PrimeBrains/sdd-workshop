...
| **レビュー担当** | reviewer (R-T5, v19) | `implemented→accepted` を行うべく指名された人間。assignee とは別の付帯属性・単一・**人間限定**・平準化/EV/PV/coverage を消費しない。指名(plan)で実承認者と異なってよい | 「自分のレビュー」絞り込みは視点 actor（提示層）で別概念。未指名は可視ギャップ |
