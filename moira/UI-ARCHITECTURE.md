...
> **本節は doc-refine ゲート未通過の追補**（2026-07-05・D-73/ADR-0005 とともに `proposed`）。
> 上の §1–§6 の被覆表・ゲート所見（確定来歴）の射程は従来どおり**単一プロジェクトの 5 サーフェス**であり、本節はそれを変更しない。敵対検証の引き継ぎは issue #23 コメント参照。
...
- **ビュー**: `案件並置`（1行1案件の会計並置＋件数のみの合計行＋loadError 行）／`人横断`（Actor.id 一致での掛け持ち見える化・開示文言常設）／ドリルダウン（選択案件の fixture で既存 `MoiraProvider`＋`WorkbenchShell` を再マウント＝§1–§6 の規律がそのまま適用される）。
