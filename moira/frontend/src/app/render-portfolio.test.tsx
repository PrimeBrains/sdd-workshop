...
describe('portfolio shell renders without throwing', () => {
  ...
  it('overview totals only unit-safe counts and discloses why', () => {
    const html = wrap(<PortfolioOverview onOpenProject={() => {}} />);
    expect(html).toContain('合計（件数のみ）');
    expect(html).toContain('横断の合成会計は行いません（D-50）');
    expect(html).toContain('EV%・SPI・CPI は案件ごとに BAC も単位も異なるため合成しません');
    expect(html).toContain('バッファ残');
    expect(html).toContain('レビュー待ち');
  });

  it('person view carries the permanent D-50 / Actor.id disclosure', () => {
