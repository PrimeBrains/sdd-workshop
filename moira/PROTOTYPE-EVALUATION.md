...
| 5 | evm-studio | **捨** | **検証済み EVM 計算骨格（標準 EVM。EV 意味論は P1 と相違）[高]**／α_i の独立到達（傍証）／派生シグナル・UX カタログ(SPI/CPIトレンド・CCPM fever・alertLevel・前日比・Gantt) **[中]** | SQLite スナップショット保存（⊥A2）／WBS 先行（A1 欠）／固定見積（§2.2 欠）／※isBuffer は v15/R-T6 で不採用に解消（A1 違反; §5.5） |
...
**設計論点（v15 で解消）**: `isBuffer` は CCPM バッファ**ノード**（留保量を `estimateDays` として保持。`bufferTotalDays = Σ(isBuffer タスクの estimateDays)`、`bufferConsumption = 累積遅延/バッファ総日数`）。**MODEL v15（R-T6）でこの論点は解消した**: バッファ*ノード*（`isBuffer`）は **A1 違反ゆえ不採用**（ノードは spec 分解のみ。バッファは宣言でなく導出）。代わりに Moira は**スケジュール・バッファを二参照日付（期日・目標日）＋ P7 導出完了からの導出**として正典化した——バッファ残量 = max(0, 期日 − 導出完了)、スケジュール・カバレッジ対読みで de-rate（R-T6/§3）。fever の消費率シグナルの出力 UX（消費% × 進捗%）は提示の自由（P0）として参照可だが、ゾーン閾値・CCPM のバッファ*生成*（安全余裕除去・集約）・工数軸（BAC×想定生産性）は不採用（MODEL §5・§7#11）。
