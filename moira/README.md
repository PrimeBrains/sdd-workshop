...
> 観測・導出・警告に徹し、コミットメントを伴う判断(見積の合意・割当・スコープ/期日)は人間に残す。**

名前の由来は μοῖρα「割り当てられた持ち分」= Earned Value/予算配賦、運命の三女神が三射影
(クロト=分解・計画／ラケシス=進捗測定／アトロポス=完了・取消)に対応する。→ [`NAMING.md`](./NAMING.md)

## 現状

- 正典モデルは **v10**(独立敵対レビュー8ラウンドを経て確定)。→ [`MODEL.md`](./MODEL.md)
