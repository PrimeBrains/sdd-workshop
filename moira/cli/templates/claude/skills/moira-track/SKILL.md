...
- ログ home は `--dir`（グローバル）→ `MOIRA_DIR` → `.moira` ポインタファイル/上位探索 → カレント の順で解決される（multi-repo・ADR-0003。単一リポは従来どおりカレント直下の `.moira/`）。本スキル・hooks・steering 発火表は `moira adapter install` が設置する（`moira adapter status` で検査）。
