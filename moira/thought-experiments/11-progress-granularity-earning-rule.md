...
- **同型の限界(開示)**: (a) 〔**v18 で解消**〕estimateCoverage(P2)は本 TE 当時は分母が**全有効ノード**(中間含む)で
  葉基底の execCov/scheduleCoverage と集合が異なったが、**v18 で P2 を葉基底へ是正**(`|合意済み有効葉|/|既知の有効葉|`)
  し、いまは同一の有効葉集合で測る(§6 v17→v18。ただし測度は execCov/schedCov=カウント比、EV%=MD 加重で異なる)。(b) 述語の**時間安定性**が異なる
