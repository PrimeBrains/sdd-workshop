# 仕様駆動・チケット駆動・EVM 統合モデル v11
# Unified Model: Spec-Driven × Ticket-Driven × EVM (v11)

> 合言葉 / Motto: **必要かつ十分 (Necessary and Sufficient)**
>
> 本書はアーキテクチャ以前の「思想の確定」であり、実装技術には立ち入らない。
> v11 は、思考実験 04–10(計 15 件:Critical 2 / Important 13)と 2 件の設計分岐(FORK)裁定を反映した版である。主な是正:キャンセル伝播と孤児警告(R-C3 新設)、半凍結ベースラインの閉塞(PV からの除外規則)、矛盾合意警告(R-U12 新設)、単一被割当者の意図的仕様化、P1 の偽の普遍命題修正、エージェントリードタイムのクリティカルパス算入(P7/R-T2)。新公理・新イベント・新状態は増やしていない。新要件 4 件(R-C3/R-S7/R-U12/R-U13)はすべて警告・可視化であり §0 と一貫。
> *v11 incorporates 15 findings (2 Critical, 13 Important) and 2 FORK resolutions from thought experiments 04–10: cancel propagation with orphan warning (R-C3), half-frozen baseline closure (PV exclusion rule), contradiction-agreement warning (R-U12), explicit single-assignee spec, P1 false-universal fix, agent lead-time on the critical path (P7/R-T2). No new axioms, events, or states. Four new requirements (R-C3/R-S7/R-U12/R-U13), all warnings or visible-gap surfacing, consistent with §0.*
...
スケジュールは、人間が与えた割当のもとでの 1.0MD 平準化として導出。エージェント作業は平準化対象外でスラックを持たず、通常は人間接点の間隙を埋める。**ただしエージェントタスクが依存連鎖上で人間タスクを律速する場合(A→H)、A のリードタイム(P6)はクリティカルパス計算に算入される**——クリティカルパスは全依存連鎖のうち最長のパスであり、人間タスクは平準化(資源制約)の対象、エージェントタスクは非平準化だがリードタイム(P6)でパス長に寄与する。
*Schedule = 1.0MD leveling over human-given assignments; agent work fills gaps. However, when an agent task rate-limits a human task on the dependency chain (A→H), A's lead time (P6) enters the critical path — the critical path is the longest path through the full DAG — human tasks are leveled (resource-constrained), agent tasks are not leveled but contribute their lead time (P6) to path length.*
...
「完全に閉じた」という宣言はしない(本モデル自身の P2「未発見は測れない」と一貫させる)。v11 確定までに思考実験 04–10 が出した Critical/Important 15 件および FORK 2 件は、是正または意図的スコープ外の明示で決着させた。以下は v11 時点で認識している残余の開示:
...
*We do not declare the model "fully closed" (consistent with the model's own P2). As of v11, all 15 Critical/Important findings and 2 FORK resolutions from thought experiments 04–10 are resolved by correction or explicit scoping-out. Residual disclosures: (1) frozen-slot selection is implementation-dependent (P8 consequence, disclosed); (2) CPI is modeling-choice-dependent (suppressed but not quantitatively bounded, disclosed); (3) granularity asymmetry between agreement/EV/transition levels (deliberate design choice, disclosed). Structure, vocabulary, and defaults are practically stable; the next stage is concrete architecture. Further gaps, if found, are folded in via the moira-model-update skill (append-only).*
