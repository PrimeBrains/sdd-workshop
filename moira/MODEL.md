# 仕様駆動・チケット駆動・EVM 統合モデル v16
# Unified Model: Spec-Driven × Ticket-Driven × EVM (v16)

> 合言葉 / Motto: **必要かつ十分 (Necessary and Sufficient)**
>
> 本書はアーキテクチャ以前の「思想の確定」であり、実装技術には立ち入らない。
> v16 は、完了主義の EV%/SPI/CPI が**仕掛中(`implementing`)に動かず執行中の進捗把握が遅れる**課題に対し、**実行カバレッジ executionCoverage**(現行有効・合意済み葉のうち `implementing` にあるもののノード数比率)を新要件 **R-S8** として追加した版である。当初検討した「`implementing` で固定割合 F の EV を稼ぐ獲得ルール(partial EV)」は独立敵対ゲートで**却下**した——状態別の乗率を EV に折り込むのは R-U8「state-weight table 不使用」・P1「重みテーブルなし」・L47「裁量ノブ不在」(v15 が全廃した留保率と算術同型)に抵触するため。実行カバレッジは **scheduleCoverage(R-S6)と構造同型の count-based coverage**(状態述語でノードを数えるだけで予算に乗率を掛けない)ゆえ R-U8 の射程外で、**EV_abs/EV%/PV/SPI/CPI の式を一切変更しない**別建ての read であり、EV% と estimateCoverage(P2)と三者対読みする(算術和して全体進捗とするのは禁止——仕掛中の*量*であって出来高ではない=R-S4/R-S6 同型の de-rate;実際の前進は R-S7 乖離・リードタイム P6 で読む)。新公理・新イベント・新状態・新原理番号は増やさず、新要件は **R-S8 一件**(R-U8/L47/P1/EV_abs/SPI/CPI/PV は不変)。独立採点者(moira-gate-judge)の残存 Critical/Important = 0。
> *v16 adds execution coverage (executionCoverage = the count ratio of agreed effective leaves currently in `implementing`) as new requirement R-S8, addressing the gap that completion-based EV%/SPI/CPI do not move during `implementing` (delaying in-execution progress visibility). The initially-considered "earning rule" (partial EV: earn a fixed fraction F at `implementing`) was rejected by the independent adversarial gate — folding a per-state multiplier into EV violates R-U8 ("no state-weight table"), P1 ("no weight table"), and L47 ("no discretionary knob"; it is arithmetically the reserve rate v15 abolished). Execution coverage is a count-based coverage structurally isomorphic to scheduleCoverage (R-S6) — it counts nodes by a state predicate and multiplies no budget by any weight — hence outside R-U8's scope, and it changes none of the EV_abs/EV%/PV/SPI/CPI formulas: a separate read, triple-paired with EV% and estimate coverage (P2) but NEVER summed as overall progress (it is in-progress VOLUME, not earned value — an R-S4/R-S6-isomorphic de-rate; actual advance is read via R-S7 divergence and P6 lead time). No new axioms/events/states/principle-numbers; one new requirement (R-S8); R-U8/L47/P1/EV_abs/SPI/CPI/PV are unchanged. Zero surviving Critical/Important per an independent judge.*
...
**A1 の射程（ノードの包含範囲）:** ここでの「spec」は特定の仕様方法論（cc-sdd 等）の成果物に限定せず、プロジェクトが追跡する**あらゆる作業単位——実際に遂行され出来高(EV_abs)を生む仕事**——を指す。運用タスク・バグ修正・ad-hoc 作業も feature ノードとして同一のイベントログ・導出・ライフサイクルに乗る。ここで「作業単位」は遂行される仕事であって、バッファのような**導出される会計量ではない**（バッファは二参照日付＋導出完了からの導出であってノードではない；§5・R-T6）。フル・フェーズ周期（§2.6 の req/design/tasks 子ノード展開）を踏むか省略するかは**分解の深さ（人間のコミット判断 §2.1#4 / P0）**であり構造的制約ではない——バグ修正はフェーズ子ノードを展開せず単一ノードとして lifecycle（§2.5 の `pending→ready→implementing→implemented→accepted`）を回して完結しうるし、運用タスクは design フェーズの展開を省略しうる。**いずれの場合も lifecycle 状態機械（§2.5）はそのまま適用され、省略されうるのは §2.6 のフェーズ子ノード展開であって lifecycle 状態ではない。**いずれもノードの形態の一つであって例外ではない。
*"Spec" here is not limited to artifacts of a particular methodology (e.g. cc-sdd) — it encompasses any work unit the project tracks, meaning work that is actually performed and earns value (EV_abs). Ops tasks, bug fixes, and ad-hoc work are feature nodes on the same log, derivations, and lifecycle. A "work unit" is performed work, NOT a derived accounting quantity such as a buffer (a buffer is derived from two reference dates + derived completion, not a node; §5, R-T6). Whether to expand the full phase cycle (§2.6's req/design/tasks child nodes) or omit it is decomposition depth (a human commitment, §2.1#4 / P0), not a structural constraint — a bug fix may complete as a single node running the lifecycle (§2.5's `pending→ready→implementing→implemented→accepted`) without expanding phase child nodes, and an ops task may omit the design-phase expansion. In every case the lifecycle state machine (§2.5) applies unchanged; what may be omitted is §2.6's phase child-node expansion, not lifecycle states. These are forms of nodes, not exceptions.*

**チケット射影の性質（A1 の帰結）:** A1「チケットは射影」から従う性質を明示する（新公理ではなく A1/A2/R-U2 の帰結の言明）。(1) 射影は**読み出し専用**——チケット側の変更はノードを直接書き換えない。これは A2/R-U2（ノードの状態変化は4イベントのみ・保存状態への直接変更を拒否）の帰結であり、外部チケットシステム側の変更を Moira に反映する場合も、それは4イベントの**再発行**（取り込みアダプタ経由）としてのみ入り、チケットからノードへの直接の逆書き込み経路は存在しない（追記専用の精神）。(2) 射影は**全射でない**——すべてのノードがチケットを必要とするわけではなく、どのノードを射影するかは実装の選定（MODEL は射影対象を規定しない）。射影のメカニズム・**多重度**（ノードあたりのチケット数、対象外部システム、API・フォーマット）は**実装の責務**であり MODEL は規定しない（マルチシステムへの同時射影や粒度調整は実装が決める；ユーザー裁定 0b「不変条件のみ・メカニズムは実装」）。
*Ticket-projection properties (consequences of A1's "tickets are projections" — not new axioms, but statements of what follows from A1/A2/R-U2): (1) projections are **read-only** — ticket-side changes do not directly rewrite nodes. This follows from A2/R-U2 (node state changes only via the four events; direct mutation rejected); when an external ticket-system change is to be reflected in Moira it enters only as a **re-emission** of the four events (via an ingestion adapter), with no direct ticket→node write-back path (append-only spirit). (2) The projection is **not surjective** — not every node needs a ticket; which nodes to project is an implementation choice (the MODEL does not prescribe projection targets). The projection mechanism and **multiplicity** (tickets-per-node, target external systems, API/format) are implementation's responsibility and not prescribed by the MODEL (simultaneous multi-system projection and granularity tuning are implementation decisions; user adjudication 0b: "invariants only; mechanism is implementation").*
...
**A4 — 単一希少資源.** 希少資源は**人間**のみ。各人間は**日次容量 c(i,d) ∈ [0, 1.0] MD/日**を持つ——暦日 d に当該プロジェクトへ投下できる時間量。c は人間が与える入力（§2.1 #5）であり、未指定日は **1.0MD/日**（後方互換）。各 c の改定は**理由付き**（契約・祝日・休暇・一時減 等）で記録する。c は当該プロジェクトへ投下できる**時間量**であり、能力（スキル・習熟度）ではない——**システムは人間の能力をモデル化しない**。それは日々成長し、伴走・教育という柔軟性で運用される領域であり、硬いルールで縛らない。承認点は §2.1 #5（容量 c の宣言）に従う。
...
3. **スコープ/期日・目標日の決定** — 期日超過時、要員追加か機能削減かは人間が決める(R-T4)。**期日(外部制約・硬い上限)と目標日(計画完了の参照点)は人間が与えるプロジェクト構成入力**であり、両者の差が人間の管理する**スケジュール・バッファ**を画定する(R-T6)。バッファの残量モニタリングはこの二参照点と導出完了から導く(§3/R-T6)が、これらは核心導出を調整しない参照点であって裁量ノブではない(§5)。
...
本システムの警告(R-U12/R-U13/R-T3/R-T4/R-S3/R-S6/R-S7/R-C3 および P5 の at-risk)はすべて**現在の導出状態**に対する述語であり、イベント追記時および構成入力(c・期日・目標日)の変更時(R-S2)に再評価される。各警告は**その条件が現在真である限り**成立し、**条件を偽化する入力(4イベントの追記 または構成入力〔c・期日・目標日〕の変更)**が起きた時にのみ消える(歴史的事実そのものではなく現在状態で判定する——例: R-U12 は「現行 latest-wins の agreed 値が actor 間で食い違う」、P5 at-risk は「implemented 到達後に後退遷移があり、まだ implemented に再到達していない」、R-T4 は「導出スケジュールが現行の期日を超える」)。
人間が警告を**見て受容し、何もしない(イベントを起こさない)ことは正当な非コミット**だが、それは条件を偽化しないため警告を**消さない**——条件が真なら警告は可視のギャップ(P0)として残るのが正直である(§0/P0。条件が依然真なのに信号を隠す dismiss/snooze は、true positive を隠蔽してアラート疲れと誤った安心を招く反パターンであり、§0/P0 に反する)。一方、人間が**コミット判断(合意・割当・スコープ/期日・目標日・再見積・再ベースライン・辺操作・キャンセル・c 宣言)を下すと、それは入力の追記**であり、条件を偽化して警告を消す。つまり「acknowledge(イベント無し)では消えず、commit(イベント)で消える」。
...
- **合意の粒度はタスク/feature ノード単位**。配下チェック項目は AI が提案・積み上げ、ノードの合意で一括確定(仕様方法論の成果物承認ゲートと一致——cc-sdd では "tasks approved" ゲート——人間のボトルネック化を回避)。
...
- これらは仕様方法論への**明示的拡張**(cc-sdd のような構造化方法論自体は見積ステップを規定しない)。ここで**「仕様方法論」とは、成果物を段階的に作成し人間が承認していく構造化された開発プロセス**を指す(cc-sdd は本モデルの検証に用いた参照例であり、Scrum・ウォーターフォール等の他方法論はそれぞれの成果物承認ゲートを §2.2 のノード合意に写像する。本モデルが具体的に検証したのは cc-sdd に対してであり、他方法論への一般化可能性を意図はするが網羅検証は主張しない——P2 と同型の正直化)。
...
仕様方法論のフェーズ成果物（cc-sdd では requirements/design/tasks）は、feature の**子ノード**として表す（フェーズ構成は方法論に応じて異なりうるが、いずれも同一の lifecycle 状態機械を再利用する。これらフェーズ子ノードを何段展開するか・省略するかは分解の深さ＝人間のコミット判断〔§2.1#4／P0〕であって構造的制約ではない；A1 射程注釈参照）。
...
- したがって二つの読みを**明示的に分離**する(R-S5):**累積EV**(EV_abs、supersede 済みを含む過去の総出来高。NAMING §7 正式語。旧称「累積稼得」と同義)と、**現行進捗**(現行有効集合=supersede されていない葉だけで測る達成率 EV%)。長寿命プロジェクトで累積 EV_abs が履歴とともに膨らんでも、現行進捗は現行有効集合で読むため希釈されない。**カバレッジ(P2)と対で読むのは現行進捗(現行有効集合の EV%)**であり、累積EV(EV_abs)は基底が異なるため coverage と独立に読む。ダッシュボードはこれらを区別表示する。
...
`coverage = Σ(合意済み見積ノード) / Σ(既知ツリーの全ノード)`。**既知ツリーのみを測り、未発見作業は原理的に測れない。** カバレッジは**現行有効集合**の既知ツリーで測り、supersede 済み旧ノードを分母に二重計上しない(累積EV(EV_abs)とは別)。低カバレッジ時の EV% は「霧の中の既知部分の達成度」にすぎない。
...
**実行カバレッジの導出(R-S8)/ Execution coverage (R-S8).**
**実行カバレッジ executionCoverage** = |現行有効・合意済み**葉**のうち lifecycle=`implementing`| / |現行有効・合意済み**葉**|(ノード数比率・scheduleCoverage と同じ葉基底;分母0→0)。完了主義の EV_abs/EV%(達成率)は仕掛中ゼロのままなので、**執行中の作業を集約レベルで可視化**するための別建ての read である。これは scheduleCoverage(R-S6)と**構造同型の coverage**——述語が「scheduled か否か(frozenSlot≠null)」→「`implementing` か否か」に変わっただけで、**予算・見積に状態別乗率を一切掛けず状態述語でノードを数える**。ゆえに EV に乗率を折り込む weight table(R-U8 が EV% 稼得について禁ずる対象;P1「重みテーブルなし」)ではなく、**EV_abs/EV%/PV/SPI/CPI の式を一切変更しない**。EV%(予算比率)・estimateCoverage(P2)と**三者併置で対読み**するが、EV% とは次元が異なる(EV% は予算、execCov はノード数)ため**算術和して全体進捗とする提示は禁止**(R-S4/R-S6 同型の de-rate)。execCov は**仕掛中の量であって出来高・進捗ではない**——`implementing` に居座るだけのタスクも算入されるため、実際の前進は生きた予測の基準乖離(R-S7)・リードタイム(P6)と対で読む。**合意済みのみ**を語り、未合意の `implementing` 葉は分子・分母外で estimateCoverage の可視ギャップ(P0)に現れる。値は lifecycle 状態(ログから決定的)に依り平準化(P7/P8)に依らない(partial EV のような P8 伝播は無い);count-based ゆえ**再見積では動かず**状態遷移時のみ変化する。**注意(忠実度):** `implementing` を記録せず `ready→implemented` にスキップする経路では doing が立たず直接 done になる——execCov の忠実度は `implementing` が記録されることに依る(凍結スロットが SPI の前提であるのと同型;§7)。
*Execution coverage = |agreed effective leaves in `implementing`| / |agreed effective leaves| (a count ratio over the same leaf basis as scheduleCoverage; empty → 0). Completion-based EV_abs/EV% stays at zero during work-in-progress, so this is a separate read that surfaces in-execution work at the aggregate level. It is structurally isomorphic to scheduleCoverage (R-S6) — the predicate is swapped (scheduled? → implementing?), it multiplies no budget by any per-state weight, it merely counts nodes — hence it is NOT a state-weight table (R-U8's prohibition is on the EV% derivation; P1 "no weight table") and it changes none of the EV_abs/EV%/PV/SPI/CPI formulas. It is triple-paired with EV% and estimateCoverage (P2) but, being a different dimension from EV% (budget vs node count), it is NEVER summed as overall progress (an R-S4/R-S6-isomorphic de-rate). It is in-progress VOLUME, not earned value or progress — a task merely parked in `implementing` still counts; its actual advance is read via R-S7 forecast divergence and P6 lead time. It speaks the agreed (committed) region only; an unagreed `implementing` leaf is outside both numerator and denominator and shows up as an estimateCoverage visible gap (P0). Its value rests on lifecycle state (log-deterministic), not on leveling (P7/P8) — no P8 contamination like partial EV — and being count-based it does NOT move on re-estimation, only on state transitions. Fidelity note: a `ready→implemented` skip path raises no doing and goes straight to done, so execution coverage's fidelity depends on `implementing` being recorded (isomorphic to frozen slots being SPI's precondition; §7).*

**スケジュール・バッファの導出(R-T6)/ Schedule buffer (R-T6).**
プロジェクトは二つの参照日付を**構成入力**として持ちうる(c と同じ第二層・追記専用・理由付き履歴・プロジェクトレベル単一; R-T6/R-U14 同型): **期日(deadline)** = 外部から課される硬い上限、**目標日(target date)** = 人間が管理する計画完了の参照点。生きた予測スケジュール(P7/P8)のプロジェクト導出完了(クリティカルパス末端の予測完了; 複数末端なら max)を D_pred とすると:
- **バッファ残量** = max(0, 期日 − D_pred)。**期日(外的事実)と P7 導出完了のみに依存**し、目標日には依存しない。D_pred が期日を超えた分は R-T4 の超過量(導出完了 − 期日)として現れ、残量は 0 にクランプされる(超過は不可視化しない——超過量は R-T4 が保持する; P0)。
- **バッファ消費率** = clamp₀¹((D_pred − 目標日) / (期日 − 目標日))(目標日 ≤ D_pred ≤ 期日 の区間で 0→1)。目標日は既に正直な量(残量)の消費%を**再基準化**するだけで、核心導出を調整しない(人間が設定する目標日に依るため、消費率の**プロジェクト間比較は無意味**——CPI のモデル化選択依存 §7#2 と同型の注意)。
- **規律(必須)**: バッファ残量・消費率は P7 導出完了に依拠し、**導出完了はスケジュール済み(割当済み)領域のみ**を覆う(P0)。ゆえに**スケジュール・カバレッジ(R-S6)と必ず対で読み、低い間は de-rate** する——未割当バックログが大きいと D_pred は楽観的に過小となり、バッファ残量が**偽の余裕**を示すため(SPI を R-S6 でデレートするのと同型の規律)。
- **境界条件**: 目標日なし → 残量は算出可(= 期日 − D_pred = 対期日スラック)だが**消費率は N/A**。期日なし → バッファ未定義(R-T4 も発火しない; v14 以来の動作)。目標日 = 期日 → 消費率 N/A(分母0)・残量のみ。目標日 > 期日 → 構成エラーとして警告(§2.1「システムは警告し人間が決める」)・バッファ N/A。D_pred が存在しない(スケジュール・カバレッジ = 0) → バッファ残量は**算出不能**として可視ギャップに出す(潰さない; P0)。
- **凍結でなく生きた予測**: バッファ残量は**生きた予測**側の量であり凍結ベースラインではない——supersede/再見積/c 変更で D_pred が動けば残量も動く(P5 同型の正直な非単調; 凍結 PV/SPI は影響を受けない)。実装依存性(D_pred は P7/P8 の発見的・非決定的導出に依拠)とドリフト(記録された期日/目標日 vs 実態)は §7#10 に開示。
*Schedule buffer (R-T6): a project may carry two reference dates as configuration inputs (second tier like c — append-only, reason-stamped, project-level single): the deadline (an externally imposed hard ceiling) and the target date (the human-managed planned-completion reference). With D_pred = the project's derived completion from the live forecast (P7/P8 predicted completion of the critical-path end; the max over multiple ends): buffer remaining = max(0, deadline − D_pred), depending only on the deadline (an external fact) and P7's derived completion (NOT on the target). Overrun beyond the deadline appears as R-T4's overrun magnitude (derived completion − deadline); remaining clamps to 0 but the overrun is never hidden (P0). Buffer consumption = clamp₀¹((D_pred − target)/(deadline − target)); the target only re-bases the consumption-% of the already-honest remaining, adjusting no core derivation (because it depends on a human-set target, cross-project consumption-% comparison is meaningless — a caveat isomorphic to CPI's modeling-choice dependence, §7#2). Discipline (mandatory): buffer rests on D_pred, which covers only the scheduled (assigned) region (P0), so it is read paired with schedule coverage and de-rated while coverage is low — a large unassigned backlog makes D_pred optimistic and the buffer falsely comfortable (isomorphic to de-rating SPI under R-S6). Boundary cases: no target → remaining computable (= slack to deadline) but consumption N/A; no deadline → buffer undefined (R-T4 also silent, as since v14); target = deadline → consumption N/A (zero denominator), remaining only; target > deadline → a config-error warning (§2.1 "system warns, human decides"), buffer N/A; no D_pred (schedule coverage = 0) → buffer remaining uncomputable, shown as a visible gap (not potted; P0). Buffer remaining is a live-forecast quantity, not a frozen baseline — it moves when D_pred moves (supersede/re-estimate/c change), an honest non-monotonicity like P5; the frozen PV/SPI are unaffected. Implementation-dependence (D_pred rests on P7/P8's heuristic, non-deterministic derivation) and drift (recorded deadline/target vs reality) are disclosed in §7#10.*
...
**R-U5.** The system shall require human agreement at the task/feature node level, rolling up checklist-item estimates to be agreed en masse, aligned with the spec methodology's deliverable-approval gate (in cc-sdd, the "tasks approved" gate).
本システムは人間の合意をタスク/feature ノード単位で要求し、配下チェック項目を一括合意として積み上げ、仕様方法論の成果物承認ゲート（cc-sdd では tasks approved ゲート）と一致させなければならない。
...
**R-S2 (Event).** When any event is appended, the system shall make the updated derived state — node states, EV% and EV_abs, estimate coverage, execution coverage (R-S8), PV (baseline), AC, SPI, CPI, queues, live forecast schedule (including each sub-unit's predicted completion), unassigned backlog, and schedule buffer remaining/consumption (R-T6) — available to all three dashboards from the same log. (A node's baseline budget fixes at agreement and its slot at first scheduling; neither moves thereafter except by explicit re-baseline of an incomplete node; the live forecast schedule is what recomputes. Re-derivation is triggered by event append AND by configuration-input (c, deadline, target date) changes (A4/R-T6), since these are not themselves events.)
イベント追記時、本システムは更新後の導出状態(各ノード状態・EV%・EV_abs・見積カバレッジ・実行カバレッジ(R-S8)・PV(ベースライン)・AC・SPI・CPI・各キュー・生きた予測スケジュール〔各サブ単位の予測完了を含む〕・未割当バックログ・スケジュール・バッファ残量/消費率〔R-T6〕)を同一ログから三ダッシュボードへ提供しなければならない(各ノードのベースライン予算は合意時、スロットは初回スケジュール載り時に確定し、以後は未完了ノードの明示的な再ベースラインを除き動かない。再計算されるのは生きた予測スケジュールであり、その契機はイベント追記に加え構成入力(c・期日・目標日)の変更(A4/R-T6)でもある——これらはイベントではないため)。
...
**R-S8 (Ubiquitous).** The system shall derive **execution coverage** — the count ratio of agreed effective leaves currently in `implementing` over all agreed effective leaves (empty denominator → 0, honest empty) — and present it paired with EV% and estimate coverage to surface, at the aggregate level, the in-execution region that completion-based EV% omits. (a) It is derived **agreed-only**: an unagreed `implementing` leaf is excluded from both numerator and denominator and appears as an estimate-coverage gap (its own clause; R-U8/R-U13 govern EV_abs/EV%, not this read). (b) It shall **not be summed with EV% as project-wide progress** — it is in-progress VOLUME, not earned value (a leaf merely parked in `implementing` still counts; its actual advance is read via R-S7 divergence and lead time, P6) — an R-S4/R-S6-isomorphic de-rate discipline. (c) It is a **count ratio over a state predicate, structurally isomorphic to scheduleCoverage (R-S6)** (predicate swapped: scheduled? → implementing?), multiplying no budget by any per-state weight; it is therefore **not a state-weight table** (R-U8's prohibition is on the EV% derivation) and **touches none of the EV_abs/EV%/PV/SPI/CPI formulas**. Its value rests on lifecycle state (log-deterministic), not on slots/leveling (P7/P8); only the *timing* of the `implementing` transition depends on operations (§7).
本システムは**実行カバレッジ**——現行有効・合意済み葉のうち `implementing` にあるものの、合意済み有効葉全体に対するノード数比率(分母0→0・honest empty)——を導出し、**EV% および見積カバレッジと対で**提示して、完了主義の EV% が落とす「執行中」領域を集約レベルで可視化しなければならない。(a) **合意済みのみ**で導出する:未合意の `implementing` 葉は分子・分母とも除外され見積カバレッジのギャップに現れる(本要件独自の条文;R-U8/R-U13 は EV_abs/EV% を律する別条)。(b) **EV% と算術和して全体進捗として提示してはならない**——これは仕掛中の*量*であって出来高ではない(`implementing` に居座るだけの葉も算入される;実際の前進は R-S7 乖離・リードタイム P6 で読む)——R-S4/R-S6 同型の de-rate 規律。(c) **状態述語上のノード数比率であり scheduleCoverage(R-S6)と構造同型**(述語が scheduled?→implementing? に変わっただけ)で、予算に状態別乗率を掛けない。ゆえに **state-weight table ではなく**(R-U8 の禁止は EV% の導出に掛かる)、**EV_abs/EV%/PV/SPI/CPI の式に一切触れない**。値は lifecycle 状態(ログから決定的)に依りスロット/平準化(P7/P8)に依らない;`implementing` 遷移の*タイミング*のみ運用に依存する(§7)。
...
**R-T6 (Ubiquitous).** The system shall treat the project **deadline** (an externally imposed hard ceiling) and **target date** (the human-managed planned-completion reference) as **project configuration inputs** — a second tier distinct from the four-event node log (A2), append-only, reason-stamped, timestamped (isomorphic to c's R-U14), project-level single — and shall re-derive on their change (R-S2). From these and the live forecast's project derived completion D_pred (P7/P8; the max over critical-path ends), the system shall derive **schedule buffer remaining = max(0, deadline − D_pred)** and **buffer consumption = clamp₀¹((D_pred − target)/(deadline − target))**, present them as monitoring read **paired with schedule coverage and de-rated while coverage is low** (isomorphic to R-S6; otherwise a large unassigned backlog yields false comfort), and shall not adjust any core derivation (EV/SPI/CPI/PV/schedule) by these inputs — they parameterize only this buffer overlay (§5). Deadline breach is warned by R-T4 (its overrun magnitude is this remaining once clamped to 0); buffer remaining is the pre-breach monitoring. The buffer is a live-forecast quantity (it moves with D_pred; not a frozen baseline). Boundary cases per §3: no target → remaining only, consumption N/A; no deadline → no buffer; target = deadline → consumption N/A; target > deadline → a config-error warning (not auto-rejected; §2.1); no D_pred → buffer uncomputable, shown as a visible gap (P0). The effort-axis (MD/MM) buffer is NOT in scope (§7#11).
本システムはプロジェクトの**期日**(外部から課される硬い上限)と**目標日**(人間が管理する計画完了の参照点)を**プロジェクト構成入力**——4イベントのノードログ(A2)とは異なる第二層・追記専用・理由付き・タイムスタンプ付き(c の R-U14 同型)・プロジェクトレベル単一——として扱い、その変更時に再導出しなければならない(R-S2)。これらと生きた予測のプロジェクト導出完了 D_pred(P7/P8; クリティカルパス末端の max)から、本システムは**スケジュール・バッファ残量 = max(0, 期日 − D_pred)**および**バッファ消費率 = clamp₀¹((D_pred − 目標日)/(期日 − 目標日))**を導出し、**スケジュール・カバレッジと対で読み、低い間は de-rate** した監視として提示し(R-S6 同型; さもなくば未割当バックログが偽の安心を生む)、これらの入力で核心導出(EV/SPI/CPI/PV/スケジュール)を一切調整してはならない——これらはこのバッファ・オーバーレイのみを画定する(§5)。期日超過は R-T4 が警告し(その超過量は本残量が 0 にクランプされた後の同量)、バッファ残量は超過前の監視である。バッファは生きた予測側の量である(D_pred とともに動く; 凍結ベースラインではない)。境界条件は §3 に従う: 目標日なし→残量のみ・消費率 N/A、期日なし→バッファなし、目標日 = 期日→消費率 N/A、目標日 > 期日→構成エラー警告(自動拒否せず; §2.1)、D_pred なし→算出不能として可視ギャップ(P0)。工数軸(MD/MM)のバッファは本要件のスコープ外(§7#11)。
...
**二層データの正直な開示（A2 の射程）:** A2「唯一のデータ＝4イベント追記専用ログ」は**ノードのワークデータ**（進捗・状態・スコープ・コスト）の単一真実源を指す。プロジェクト構成入力 c(i,d) はそれとは別の第二の永続層であり、自前の追記専用・理由付き履歴（R-U14）を持つ——v12 で α_i について既に成立していた区別を v13 が per-date の c へ一般化したもので、新たに A2 を破るのではない。c はノードの属性ではない（A1）ため4イベントに載せず（第5イベントを足さない）、本節「4イベントで閉じる」は**ノード領域について**閉じる、と読む。**期日・目標日（R-T6, v15）も同じ第二層のプロジェクト構成入力**である——c が人間資源の属性であるのに対し、これらは**プロジェクト全体の属性**という別カテゴリだが、いずれも4イベントの外にある追記専用・理由付き履歴という点で同型であり、A2 を新たに破らない（第5イベントを足さない）。これらはプロジェクトレベル単一値ゆえノードの latest-wins（I3）が直接は乗らないが、矛盾する宣言は I3 同型の `(ts,id)` latest-wins で現行値を決め、**人間内部の権威裁定はスコープ外**（§2.1）として人間が解決する（c が R-U14 で確立した形式の踏襲）。
*Two-tier data (honest scope of A2): A2's "single data = the four-event append-only log" refers to NODE work data (progress, state, scope, cost). The configuration input c(i,d) is a second persistent tier with its own append-only, reason-stamped history (R-U14) — v13 generalizes to per-date c the carve-out already true for α_i in v12; it does not newly break A2. c is not a node attribute (A1), so it is not one of the four events (no fifth event added); "closed under four events" here is read as closed over the node domain. The deadline and target date (R-T6, v15) are the SAME second tier of project configuration input — unlike c (an attribute of the human resource) they are project-wide attributes, a different category, but isomorphic in being append-only, reason-stamped history outside the four events (no fifth event added; A2 not newly broken). Being project-level singletons they do not ride node latest-wins (I3) directly, but conflicting declarations resolve via I3-isomorphic `(ts,id)` latest-wins, with authority WITHIN the human set out of scope (§2.1), resolved by humans — following the form R-U14 established for c.*

**スケジュール・バッファの必要性と非裁量性（v15）:** ユーザーの全体要件「プロジェクト全体で管理しているバッファの残量をモニタリング」を満たすため、期日（外部制約）と目標日（計画参照）を構成入力として形式化し、バッファ残量 = max(0, 期日 − 導出完了) を規律付き導出量とした。**なぜ裁量パラメータ（L47）でないか:** バッファ残量は期日（外的事実＝契約・市場制約、c と同型）と P7 導出完了のみに依存し、核心会計（EV/SPI/CPI/PV/スケジュール/カバレッジ）を一切読まず一切調整しない——別建ての可視オーバーレイにすぎない。全廃した留保率（L377）は各見積に織り込まれる**暗黙の乗率**で、水増しと実進捗を分離不能にし EV/カバレッジを歪めた（P0 違反）が、期日・目標日は核心会計を読まない**明示的参照点**であり、変化させても EV/SPI は不変・バッファという別量のみが動く。目標日は消費%の再基準化にのみ効き、主たる監視量である残量は目標日に依存しない。**なぜ CCPM でないか:** CCPM のバッファ*監視*の着想（消費の早期可視化）は借りるが、CCPM のバッファ*生成*（個別タスクの安全余裕の能動的除去とプロジェクト末尾への集約）は採らない——Moira の見積は proposed/agreed のみで「積極/安全」の積極度区分を持たず、バッファは安全余裕の集約産物ではなく二参照日付の差として定義される。フィーバーチャート的可視化（消費% × 進捗%）は提示の自由（P0）であり、ゾーン閾値（GREEN/YELLOW/RED）は裁量パラメータゆえ正典に持たない（採否は §7#11 の工数軸とともに開示）。
*Schedule-buffer necessity and non-discretion (v15): to satisfy the user's system requirement ("monitor the remaining buffer the user manages across the project"), the deadline (external constraint) and target date (planning reference) are formalized as configuration inputs, and buffer remaining = max(0, deadline − derived completion) is a disciplined derived quantity. Why not a discretionary parameter (L47): buffer remaining depends only on the deadline (an external fact — contract/market constraint, like c) and P7's derived completion; it reads and adjusts NO core accounting (EV/SPI/CPI/PV/schedule/coverage) — it is a separate, visible overlay. The abolished reserve rate (L377) was an IMPLICIT multiplier folded into each estimate, making padding inseparable from real progress and distorting EV/coverage (a P0 violation); deadline/target are EXPLICIT references the core accounting never reads — changing them leaves EV/SPI unchanged and moves only the separate buffer quantity. The target affects only the consumption-% re-basing; the remaining (the primary monitored quantity) does not depend on it. Why not CCPM: Moira borrows CCPM's buffer-MONITORING idea (early visibility of consumption) but not its buffer-GENERATION (actively stripping per-task safety margins and aggregating them at the project tail) — Moira's estimates are proposed/agreed only, with no aggressive/safe degree, and the buffer is the gap between two reference dates, not an aggregate of stripped margins. Fever-chart-style visualization (consumption-% × progress-%) is presentation freedom (P0); zone thresholds (GREEN/YELLOW/RED) are discretionary parameters and are NOT in canon (their (non-)adoption is disclosed with the effort axis in §7#11).*

**意図的に持たないもの:**
- 審議過程 → `cause` が指す spec 文書の責務。
- 重み/留保率/能力(スキル・習熟度)→ 全廃（c は外的事実入力であり本項の廃止対象ではない。上記参照）。
- 五つの判断（見積合意・割当・スコープ/期日/目標日・見積の深さ・c の契約成分の宣言）の自動化 → 持たない(P0/§2.1)。期日・目標日の決定は #3 に含まれる（R-T6）。
...
- CCPM のバッファ**生成**（個別タスクの安全余裕の能動的除去＋プロジェクト末尾への集約）→ 持たない（見積は proposed/agreed のみで積極度区分なし）。バッファ*監視*の着想のみ借用し、バッファは二参照日付の差として定義（R-T6・上記）。
- `isBuffer` ノード → 持たない（A1 射程：ノードは**遂行され出来高を生む作業**であり、バッファは作業でなく二参照日付＋導出完了からの**導出会計量**ゆえノード化は単一真実源を二重化する。A1 の「作業単位」射程はこの導出量を含まない＝A1 射程注釈と整合）。PROTOTYPE-EVALUATION §5.5 の isBuffer 保留はこれにより**不採用として解消**。
- バッファのゾーン閾値（GREEN/YELLOW/RED）→ 持たない（裁量パラメータ。可視化は提示の自由 P0、生データ＋会計で語る）。
- 工数軸（MD/MM）のバッファ（BAC×想定生産性の逆数）→ **現時点では正典に持たない**（想定生産性パラメータが L47/L33/A6 と衝突。認識済みの未採用拡張として §7#11 に忠実な実現路とともに開示）。

---

## 6. レビュー来歴 / Review Provenance

**v16 Phase 0(正典分解の前提整備・編集的ハードニング / moira-model-update スキル):** MODEL v16 を CQRS で読 spec×10／書 skill×14 に分解する(`.kiro/steering/roadmap.md`)前提として、分解がモデルに触れる論点を先に決着させた。**版番号は据え置き(v16)**——新公理・新イベント・新状態・新要件・新原理番号を増やさず、導出・数式・状態機械を一切変更しない**編集的明確化と方法論非依存化**に限るため。ユーザー裁定 FORK 4 件:**0a** A1 射程注釈(「spec」=遂行され出来高を生むあらゆる作業単位。運用/バグ/ad-hoc も feature ノード。フェーズ周期の省略は §2.6 子ノード展開の分解深さであって lifecycle 状態〔§2.5〕の省略ではない。バッファ等の導出会計量は「作業単位」に含まれず isBuffer 却下〔§5〕と整合)、**0b** チケット射影の性質を A1/A2/R-U2 の帰結として明示(読み出し専用=逆書き込み経路なし・外部変更は取り込みアダプタ経由の4イベント再発行のみ／全射でない／多重度・メカニズムは実装)、**0c** cc-sdd 固有参照(A4・§2.2・§2.3・§2.6・R-U5)を方法論非依存に一般化(cc-sdd は検証参照例として括弧内保持・「仕様方法論」を操作的に定義)、**0d** 用語確定(累積稼得→**累積EV**・コミット行為〔UI 語、正典語=コミット判断〕・実行カバレッジ〔R-S8〕を NAMING §7 正式語化)。
独立敵対ゲート(round 1: moira-adversary×3〔V1–V6〕→ 著者パッチ → moira-gate-judge)。3 敵対者が検出した Critical 3 件——(C1) A1 射程拡大が §5 isBuffer 却下根拠を自己矛盾化 → A1 を「遂行され出来高を生む作業」に限定し isBuffer 却下を導出会計量の二重化として再接地、(C2) A1 射程例示 `implementing→accepted` が §2.5 状態機械に非在 → 省略対象はフェーズ子ノード展開であって lifecycle 状態でないと明確化、(C3) 射影「高々一対一」がマルチチケットシステムで破綻 → cardinality を撤去し多重度を実装責務化(0b「メカニズムは実装」に整合)——をいずれもパッチで解消。Important(コミット行為↔コミット判断の整合・DECISIONS.md 陳腐化 cc-sdd・MODEL 本文の累積稼得残存・射影性質の公理インフレ・NAMING 命名原則の §0 抵触・NAMING 同期条項欠落・validation S4 の R-S8/R-T6 固有シナリオ欠如・UI-ARCH R-S8 三者対読み host・A4 §2.1 参照過広・R-E2b 引用)も是正。派生文書(NAMING.md §7・UI-ARCHITECTURE.md 被覆表 v14→v16・validation-scenarios.md v10→v16・DECISIONS.md)を同期。独立採点者(moira-gate-judge)の残存 Critical/Important = 0。
*v16 Phase 0 (pre-decomposition hardening; editorial, no version bump): before decomposing v16 into 10 read-specs × 14 write-skills (roadmap.md), the methodology-touching points were settled. No new axioms/events/states/requirements/principle-numbers; derivations, formulas, and the state machine are unchanged — only editorial clarification and methodology-independence. Four user-adjudicated forks: 0a A1-scope annotation (any performed, value-earning work unit; ops/bug/ad-hoc are feature nodes; phase-cycle omission is decomposition depth of §2.6 child nodes, not lifecycle-state omission; derived accounting quantities like buffers are excluded, consistent with the isBuffer rejection in §5); 0b ticket-projection properties stated as consequences of A1/A2/R-U2 (read-only, no write-back path, external changes re-enter only as four-event re-emission via an ingestion adapter; not surjective; multiplicity/mechanism are implementation); 0c generalize cc-sdd-specific references (A4, §2.2, §2.3, §2.6, R-U5) to methodology-independent language (cc-sdd retained parenthetically as the validated reference example; "spec methodology" operationally defined); 0d terminology (累積稼得→累積EV; コミット行為 as the UI term with コミット判断 the canonical term; execution coverage R-S8) canonized in NAMING §7. Independent gate (round 1: moira-adversary ×3 on V1–V6 → author patch → moira-gate-judge) found three Criticals — (C1) the broadened A1 scope contradicted the §5 isBuffer rejection → A1 narrowed to "performed, value-earning work" and the rejection re-grounded as double-sourcing a derived quantity; (C2) the A1 example `implementing→accepted` is not in the §2.5 state machine → clarified that what is omitted is phase child-node expansion, not lifecycle states; (C3) "at most one-to-one" projection breaks under multi-ticket systems → cardinality removed, multiplicity made an implementation responsibility (consistent with 0b) — all resolved by patch, with Importants corrected and derivative docs synced. Zero surviving Critical/Important per the independent judge.*

**v15→v16(実行カバレッジの正典化・獲得ルールの却下 / moira-model-update スキル):** ユーザー要件「二値 EV(完了まで0→満額)だと執行中の進捗把握が遅れる」に対し、当初案 **P-EARN(`implementing` で固定割合 F の partial EV を稼ぐ獲得ルール)** を独立敵対ゲート(round 1: moira-adversary×3＋fact-checker)が**却下**——F は状態→重み写像で R-U8「no state-weight table」/P1「重みテーブルなし」に該当し、EV に折り込む乗率で L47/§7#11(留保率との算術同型)に抵触、かつ分解(P1)で除去可能、PV 非対称・下方再見積で EV%>1 等の未処理多数。fact-checker は 0/100・50/50・20/80 が標準 EVM の客観手法であることを NO_OBJECTION で裏取りしたが、Moira の L47/R-U8 は標準より厳格(標準が保持する重み表を意図的に溶かした)ため支えにならないと確認。
**ユーザー裁定 FORK(canon-home):** 「正典 R-S 要件 / 実装のみ / §7 開示」を AskUserQuestion に回し、ユーザーは**正典 R-S 要件**を選択。partial EV を使わず核心会計を読み変えない別建てオーバーレイ **実行カバレッジ executionCoverage**(scheduleCoverage 同型の count-based coverage)を **R-S8** として追加。
中核の是正(著者がパッチ): (i) 定義を **count-based** 化(scheduleCoverage と構造同型→R-U8 反証が自明・estimateCoverage と次元一致・再見積非単調も消滅)。(ii) **R-U8 該当(FORK1)を scheduleCoverage 構造同型で反証**し、別 adversary の再反論で**健全と確認**(round 3)——R-U8 の射程は EV% 稼得への state-weight 適用で、EV 式に触れない coverage 類は射程外(でなければ R-S6/P2 自体が違反になり正典が自己矛盾)。(iii) de-rate 規律(出来高でなく仕掛中量・EV% との算術和禁止・R-S7/P6 と対読み)、合意済みのみ(execCov 独自条文)、分母0=0、スキップ/未合意は P0 可視ギャップ、を §3/R-S8 に明記。(iv) round2 の「done/doing/todo 算術分割」主張は撤回。R-S2 に executionCoverage を追加、§3 に定義、§7#13 に残余開示。思考実験は `moira/thought-experiments/11-progress-granularity-earning-rule.md`。
新公理・新イベント・新状態・新原理番号は増やしていない。新要件は **R-S8 一件**。R-U8/L47/P1/EV_abs/SPI/CPI/PV は不変。独立採点者(moira-gate-judge)の残存 Critical/Important = 0。
*v15→v16 canonizes execution coverage and rejects the earning rule. The initial proposal P-EARN (earn a fixed fraction F of EV at `implementing`) was rejected by the independent gate: F is a state→weight map violating R-U8 ("no state-weight table") and P1 ("no weight table"), an EV-folded multiplier colliding with L47/§7#11 (arithmetically the abolished reserve rate), removable via decomposition (P1), with many unhandled cases (PV asymmetry, EV%>1 on downward re-estimation). The fact-checker confirmed 0/100, 50/50, 20/80 are standard objective EVM rules (NO_OBJECTION) but Moira's L47/R-U8 is deliberately stricter (it dissolved the weight tables standard EVM keeps). User-adjudicated fork (canon-home): execution coverage is adopted as a canonical requirement (R-S8) — a count-based coverage isomorphic to scheduleCoverage that reads, but never rewrites, core accounting. Core fixes: count-based definition (isomorphic to scheduleCoverage → the R-U8 rebuttal is self-evident, dimension matches estimateCoverage, re-estimation non-monotonicity disappears); the R-U8 applicability (FORK1) was refuted via the scheduleCoverage isomorphism and confirmed sound by a re-challenge adversary (round 3) — R-U8's scope is the EV% derivation, not EV-independent coverages (else R-S6/P2 would themselves violate it and the canon would be self-contradictory); a de-rate discipline (in-progress VOLUME not earned value, never summed with EV%, paired with R-S7/P6), agreed-only (its own clause), empty→0, skip/unagreed as P0 visible gaps. No new axioms/events/states/principle-numbers; one new requirement (R-S8); R-U8/L47/P1/EV_abs/SPI/CPI/PV unchanged. Thought experiment: `moira/thought-experiments/11-progress-granularity-earning-rule.md`. Zero surviving Critical/Important per an independent judge.*

**v14→v15(スケジュール・バッファのモニタリングを正典化 / moira-model-update スキル):** ユーザーの全体要件「プロジェクト全体で管理しているバッファの残量をモニタリング」を受け、当初案「CCPM 概念(クリティカルチェーン・プロジェクトバッファ・フィーバーチャート)を §3 に導出として追加」を独立敵対ゲートで検証した結果、全面的に棄却された——CCPM バッファ(安全余裕除去の産物)≠ Moira スラック、5項目すべてが P7/R-S7/R-T4 と冗長、§5 留保率全廃と矛盾、クリティカルチェーン同一性が P8 非決定性で不安定。著者は反証でなく**過剰主張を撤回**し問題を再フレームした。
敵対レビュー(Round 1–3、各 moira-adversary ×3〔V1–V6〕＋ moira-fact-checker、計12敵対派遣＋3事実検証): fact-checker は EAC=BAC/CPI の標準性・management reserve の PMB 外性・留保率(タスク padding)と MR の区別・CPI 早期不安定性(Christensen & Heise)を裏取りし、「BAC×想定生産性=工数バッファ」が COCOMO(冪乗則 b>1)とは異なること・MM が Brooks の「神話の単位」であることを CORRECTED した。
中核の是正(著者がパッチ): (i) 当初の CCPM 導出追加を撤回し、唯一の実構造ギャップ＝**R-T4 が参照する外部期日の入力未形式化**を R-T6 で形式化。(ii) **目標日**を加え二参照点とし、**バッファ残量 = max(0, 期日 − 導出完了)**を §3/R-T6 の規律付き導出量とした(スケジュール・カバレッジ対読みで de-rate＝R-S6 同型、偽の安心の防止)。(iii) L47 非抵触の論証＝バッファ残量は期日(外的事実)と導出完了のみに依存し核心会計を読まず調整しない・目標日は消費%の再基準化のみ(全廃した留保率は EV/カバレッジに織り込む暗黙乗率だった点で質的に異なる)。(iv) R-T4 を R-T6 と表裏に整合(警告 vs 監視)、R-S2 に再導出契機と導出項目を追加、§2.1 警告持続トリガーに期日/目標日変更を追加、§0/§2.1#3/§5 を整合。(v) 境界条件(目標日なし/期日なし/目標日=期日/目標日>期日/D_pred なし)を §3/R-T6 に明記。
**ユーザー裁定 FORK:** (a) バッファ正典化の形＝「二参照点＋規律付き導出量」、(b) 粒度＝プロジェクト単一(ノード/フェーズ別は §7#12 開示)、(c) 工数軸(MD/MM・BAC×想定生産性)＝L47/L33/A6 と衝突するため「§7 に拡張開示・今回は時間軸のみ」。工数軸の想定生産性パラメータは留保率と算術同型で MODEL の「裁量ノブ不在」を崩すため不採用とし、忠実な実現路(工数枠の外的事実宣言＋latest 駆動 EAC)を §7#11 に記録した。
新公理・新イベント・新状態・新原理番号は増やしていない。新要件は **R-T6 一件**(既存の P7 導出完了＋二参照日付の上のバッファ監視)。独立採点者(moira-gate-judge)の残存 Critical/Important = 0。
*v14→v15 canonizes schedule-buffer monitoring. The initial proposal (add CCPM concepts — critical chain / project buffer / fever chart — as §3 derivations) was wholly rejected by the independent adversarial gate: the CCPM buffer (a product of safety-margin removal) ≠ Moira slack; all five items were redundant with P7/R-S7/R-T4; it contradicted §5's reserve abolition; and critical-chain identity is unstable under P8 non-determinism. The author retracted the over-claim and re-framed. Across Rounds 1–3 (moira-adversary ×3 on V1–V6 + a fact-checker each; 12 adversary dispatches + 3 fact-checks), the only real structural gap proved to be the un-formalized external deadline that R-T4 references — closed by R-T6, which formalizes deadline + target date as configuration inputs and derives buffer remaining = max(0, deadline − derived completion) as a disciplined quantity (de-rated paired with schedule coverage, R-S6-isomorphic). L47 is respected because buffer remaining depends only on the deadline (an external fact) and the derived completion and adjusts no core accounting (unlike the abolished reserve rate, an implicit multiplier folded into EV/coverage). User-adjudicated forks: buffer shape = two reference points + a disciplined derived quantity; granularity = project-level single (node/phase deferred, §7#12); the effort axis (MD/MM, BAC × inverse productivity) is NOT adopted (its productivity parameter is arithmetically the abolished reserve rate and breaks the "zero-knob" identity; faithful path recorded in §7#11). No new axioms/events/states/principle-numbers; one new requirement (R-T6). Zero surviving Critical/Important per an independent judge.*
...
「完全に閉じた」という宣言はしない(本モデル自身の P2「未発見は測れない」と一貫させる)。v13 確定までに、α_i スカラーの c(i,d) 一般化で独立敵対者の Critical 群（容量の二因子分離の冗長性・圧縮勤務での破綻・cause-agnostic R-S7 と旧開示の衝突 等）を決着させた（中核分岐＝単一容量 c か二因子か、はユーザー裁定で単一 c）。v14 では、警告の持続・消滅意味論を §2.1 で明文化し、confirm/受容型解決の表現問題を決着させた(警告は現在状態の述語として条件が真の間だけ持続し、acknowledge 可変状態を持たない＝α 裁定)。v15 では、スケジュール・バッファのモニタリング(期日・目標日の形式化＋バッファ残量の規律付き導出 R-T6)を正典化し、当初の CCPM 導出追加案を棄却した(バッファは二参照日付の差であって安全余裕の集約ではない)。v16 では、実行カバレッジ(R-S8)を正典化し、当初の partial EV 獲得ルール案を棄却した(EV に状態別乗率を折り込むのは R-U8/P1/L47 抵触;execCov は scheduleCoverage 同型の count-based read で核心会計に非干渉)。以下は v16 時点で認識している残余の開示:
...
10. **スケジュール・バッファの実装依存とドリフト**(v15):(a) バッファ残量・消費率は P7/P8 の生きた予測完了 D_pred に依拠するため**実装依存**であり(同一ログでも平準化実装が異なれば D_pred が変わる; §3「同一ログ・同一実装に対してのみ保証」、SPI の #1 と同型)、実装間のバッファ値の比較可能性は保証されない。(b) 記録された期日・目標日と実態の**ドリフト**(外部期日が口頭で動いたが未記録 等)は構造的トリガーを持たず、認識と更新は P0＋R-T6 の理由付き履歴に委ねる(c の drift #4(b) と対称)。(c) 消費率は人間設定の目標日に依存するため**プロジェクト間の消費%比較は無意味**(CPI のモデル化選択依存 #2 と同型)。

11. **工数軸(MD/MM)のバッファと想定生産性**(v15・認識済みの未採用拡張):ユーザーは「BAC×想定生産性の逆数で工数バッファを MD/MM 管理する」UC を提示したが、**現時点では正典に持たない**。理由——想定生産性(=計画 CPI)を BAC に掛ける乗率は §5 L377 で「還元可能ゆえ全廃」とされた留保率と算術同型で、人間が設定して導出量(工数バッファ残量)を動かす点で L47「裁量パラメータ不在」と衝突する;工数上限を予算総額次元に置くことは「予算総額は不変(BAC)」とする L33/§3 と衝突する;EAC=BAC/CPI は §3 既存の latest 駆動 EAC と二重定義になり、CPI 序盤不安定で序盤の値が無意味になる;MM は Brooks が「有用な作業を man-month で測れるのは神話」と断じた単位で A6(アテンション時間単一通貨)と緊張する。**忠実な実現路(将来 UC 化した場合):** 想定生産性をパラメータ化せず、(i) **工数枠(envelope)を MD で直接宣言する外的事実入力**(資金枠・組織コミットメント;期日が外的時間制約であるのと対称、EVM の management reserve は PMB 外で正当)とし、(ii) 生きた予測は §3 既存の **latest 駆動 EAC** を用い(新たな CPI 駆動公式を足さない)、(iii) 工数バッファ残量 = max(0, 工数枠 − EAC)、(iv) 超過量(EAC − 工数枠)を R-T4 同型に伴わせ 0 にクランプして隠さない、(v) BAC の基底(凍結ベースライン予算か最新見積か)を FORK として明示、(vi) MM は MD の表示 view(Brooks 注記付き)。BAC×想定生産性 は工数枠を**入力時に算出する手段**(組織暦展開 §5 と同型の入力編集責務)であって正典のノブではない。バッファのゾーン閾値(GREEN/YELLOW/RED)も同様に裁量ゆえ持たない。

12. **バッファの粒度(ノード/フェーズ別)**(v15):期日・目標日・バッファはプロジェクトレベル単一(ユーザー裁定)。大規模ではフェーズ/マイルストーン別の期日・バッファが常態だが、本版では持たない。木(A3、I1 の親=Σ子集約)の上にノード別バッファを乗せる拡張は可能だが、per-node の期日/目標日入力・中間クリティカルパス末端の導出・集約規則を要し構成入力が肥大化するため、現時点では最小性を優先して見送る(P0; 必要が UC 化すれば追記的に拡張)。

13. **実行カバレッジの解像度と忠実度**(v16):(a) execCov は **count-based**(ノード数比率)ゆえ **effort を重み付けしない**——巨大な仕掛葉も微小な仕掛葉も同じ1カウント(scheduleCoverage/estimateCoverage と同性質の既知の粗さ;予算加重は R-U8/L47 と衝突するため意図的に採らない)。(b) 忠実度は `implementing` が記録されることに依る——`ready→implemented` にスキップする経路(§2.5 は例示)では doing が立たず直接 done になる(凍結スロットが SPI の前提であるのと同型)。(c) execCov は**生きた状態**の量で `implementing` の前進/後退に追従して変動する(凍結属性の scheduleCoverage より変動的);値は lifecycle に依り平準化(P7/P8)に依らないが、`implementing` 遷移の*タイミング*は運用・スケジュール進行に間接依存する。

構造・語彙・既定値は実用上安定しており、次の段階として本モデルを前提とした具体的アーキテクチャの検討に進める。今後さらなる綻びが見つかれば、本書は moira-model-update スキルを通して追記的に更新される(追記専用の精神)。
*We do not declare the model "fully closed" (consistent with the model's own P2). As of v15, residual disclosures: (1) frozen-slot selection is implementation-dependent; (2) CPI is modeling-choice-dependent; (3) granularity asymmetry; (4) recorded-c CHANGES that invalidate a baseline are caught by R-S7, but recorded-vs-reality DRIFT has no structural trigger (P0 + R-U14); (5) unspecified-day c defaults to 1.0 (missing-calendar optimism); (6) a broad c change can stale many slots at once; (7) point-in-time forecast reproduction needs valid-time c snapshots; (8) permanent departure has two encodings (unassignment vs persistent c=0); (9) warnings are current-state predicates that persist while true and clear only when an input (a four-event append or a configuration-input [c / deadline / target] change) falsifies the condition, with NO acknowledge/dismiss mutable state (minimality), an R-U14-isomorphic acknowledgment tier (β) being the disclosed upgrade path; (10) schedule-buffer remaining/consumption (R-T6) is implementation-dependent (rests on P7/P8's D_pred, isomorphic to #1) and has a recorded-vs-reality drift with no structural trigger (P0 + R-T6 history), and consumption-% is not comparable across projects (depends on a human-set target, isomorphic to #2); (11) the effort-axis (MD/MM) buffer (BAC × inverse expected productivity) is a recognized but NOT-adopted extension — the productivity parameter is arithmetically the abolished reserve rate and collides with L47/L33/A6 (MM is Brooks's "myth" unit); the faithful path, if it becomes a UC, is a directly-declared effort-envelope external fact + a latest-driven EAC (no CPI-based formula, no productivity knob), with buffer remaining = max(0, envelope − EAC) and the overrun carried (R-T4-isomorphic); (12) buffer granularity is project-level single (node/phase-level deferred; the tree could carry per-node buffers but the configuration input would bloat, so minimality is preferred for now); (13) execution coverage (R-S8) is count-based (effort-blind, like the other coverages; budget-weighting would collide with R-U8/L47), its fidelity depends on `implementing` being recorded (a `ready→implemented` skip goes straight to done), and it is a live-state quantity (more volatile than the frozen scheduleCoverage). Structure, vocabulary, and defaults are practically stable; the next stage is concrete architecture.*
