# 仕様駆動・チケット駆動・EVM 統合モデル v15
# Unified Model: Spec-Driven × Ticket-Driven × EVM (v15)

> 合言葉 / Motto: **必要かつ十分 (Necessary and Sufficient)**
>
> 本書はアーキテクチャ以前の「思想の確定」であり、実装技術には立ち入らない。
> v15 は、プロジェクトの**期日(deadline)と目標日(target date)を構成入力として形式化**し、**スケジュール・バッファの残量モニタリング**(導出完了予測と期日の間のギャップ)を、スケジュール・カバレッジと対で読む規律付き導出量として §3/R-T6 に追加した版である。期日は外部制約(契約・市場)の入力(c と同じ第二層の構成入力)、目標日は人間が管理する計画完了の参照点であり、両者は核心導出(EV/SPI/CPI/PV/スケジュール)を一切調整しない——**バッファ残量 = max(0, 期日 − 導出完了)は期日(外的事実)と P7 導出完了のみに依存し、目標日は既に正直な量の消費%を再基準化するだけ**で、別建ての可視オーバーレイにすぎず L47「裁量パラメータ不在」と整合する(全廃された留保率は EV/カバレッジに織り込まれ進捗を歪めた**暗黙の乗率**であったのに対し、期日・目標日は核心会計を読まない**明示的参照点**である)。バッファ残量は P7 導出完了に依拠するためスケジュール・カバレッジが低いとき楽観に振れる——ゆえに SPI(R-S6)と同型に**カバレッジ対読みで de-rate**する(偽の安心の防止)。工数軸(MD/MM)のバッファ(BAC×想定生産性の逆数)は、想定生産性パラメータが L47(裁量ノブ不在)/L33(予算総額不変)/A6(アテンション時間単一通貨)と衝突するため**正典には採らず、§7#11 に認識済みの未採用拡張として開示**する(忠実な実現路＝工数枠の外的事実宣言＋latest 駆動 EAC)。新公理・新イベント・新状態・新原理番号は増やさず、新要件は **R-T6 一件**(既存構造の上のバッファ監視)。CCPM のバッファ*監視*の着想は借りるが、*生成*(タスク安全余裕の除去・集約)は採らない(isBuffer ノードは A1 違反ゆえ不採用)。独立採点者(moira-gate-judge)の残存 Critical/Important = 0。
> *v15 formalizes the project deadline and target date as configuration inputs and adds schedule-buffer-remaining monitoring (the gap between derived completion and the deadline) as a disciplined derived quantity read paired with schedule coverage (§3/R-T6). The deadline is an external-constraint input (a second-tier configuration input like c); the target date is the human-managed planned-completion reference. Neither adjusts any core derivation (EV/SPI/CPI/PV/schedule) — buffer remaining = max(0, deadline − derived completion) depends only on the deadline (an external fact) and P7's derived completion, and the target date merely re-bases the consumption-% of an already-honest quantity. They parameterize a separate, explicit, visible buffer overlay, consistent with L47 ("no discretionary tuning parameters"): the abolished reserve rate was an IMPLICIT multiplier folded into EV/coverage that distorted progress, whereas deadline/target are EXPLICIT references the core accounting never reads. Because buffer remaining rests on P7's derived completion, it skews optimistic when schedule coverage is low, so it is de-rated paired with coverage exactly as SPI is (R-S6) — preventing false comfort. An effort-axis (MD/MM) buffer (BAC × inverse expected productivity) is NOT adopted into canon because the productivity parameter collides with L47 (no-knob) / L33 (budget-total invariant) / A6 (single attention-time currency); it is disclosed in §7#11 as a recognized, not-yet-adopted extension (faithful path = a declared effort-envelope external fact + a latest-driven EAC). No new axioms, events, states, or principle-numbers; one new requirement (R-T6, buffer monitoring over existing structure). Moira borrows CCPM's buffer-monitoring idea but not its buffer-generation (task safety-margin removal/aggregation; an isBuffer node is rejected as an A1 violation). Zero surviving Critical/Important per an independent judge.*
...
イベント追記時、本システムは更新後の導出状態(各ノード状態・EV%・EV_abs・見積カバレッジ・PV(ベースライン)・AC・SPI・CPI・各キュー・生きた予測スケジュール〔各サブ単位の予測完了を含む〕・未割当バックログ・スケジュール・バッファ残量/消費率〔R-T6〕)を同一ログから三ダッシュボードへ提供しなければならない(各ノードのベースライン予算は合意時、スロットは初回スケジュール載り時に確定し、以後は未完了ノードの明示的な再ベースラインを除き動かない。再計算されるのは生きた予測スケジュールであり、その契機はイベント追記に加え構成入力(c・期日・目標日)の変更(A4/R-T6)でもある——これらはイベントではないため)。
