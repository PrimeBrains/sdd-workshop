# 仕様駆動・チケット駆動・EVM 統合モデル v12
# Unified Model: Spec-Driven × Ticket-Driven × EVM (v12)

> 合言葉 / Motto: **必要かつ十分 (Necessary and Sufficient)**
>
> 本書はアーキテクチャ以前の「思想の確定」であり、実装技術には立ち入らない。
> v12 は、部分参加チーム（0.2MD/日〜1.0MD/日混在）への正直なスケジューリングを実現するため、A4 の固定容量 1.0MD/日 を人間ごとの割当率 α_i ∈ (0, 1.0] MD/日 にパラメータ化した版である。主な修正: A4(α_i 導入・AC 関係・記録・スコープ境界)、A5(資源非対称に α_i 明記)、P7/R-U11/R-T1/R-T3(1.0MD→α_i 平準化)、R-U14(α_i 変更追跡要件の新設)、L40(裁量パラメータ vs 外的制約入力の区別を明確化)、§2.1(#5 追加)、§5(α_i 必要性論証・スコープ外宣言)。新公理・新イベント・新状態は増やしていない。新要件 1 件(R-U14: α_i 変更追跡)。
> *v12 parameterizes the fixed 1.0MD/day capacity in A4 into per-human allocation rate α_i ∈ (0, 1.0] MD/day, enabling honest scheduling for fractional-allocation teams. Key changes: A4 (α_i definition, AC relationship, recording, scope boundary), A5 (α_i in resource asymmetry), P7/R-U11/R-T1/R-T3 (1.0MD→α_i leveling), R-U14 (new: α_i change tracking), line 40 (discretionary tuning vs external constraint), §2.1 (#5 added), §5 (α_i necessity, scope-out declaration). No new axioms, events, or states. One new requirement (R-U14).*
...
**A4 — 単一希少資源.** 希少資源は**人間**のみ。各人間は**割当率 α_i ∈ (0, 1.0] MD/日**の容量を持つ。α_i は人間が与える入力（§2.1 #5）であり、未指定時は **1.0MD/日**（後方互換）。α_i は当該プロジェクトに投下可能な**時間の割合**であり、能力（スキル・習熟度）ではない——**システムは人間の能力をモデル化しない**。それは日々成長し、伴走・教育という柔軟性で運用される領域であり、硬いルールで縛らない。承認点は cc-sdd ゲートに従う。
**α_i と AC の関係:** α_i は**計画上の容量上限**であり、AC（実績コスト）の記録を制約しない。AC は実際に費やしたアテンション時間を正直に記録する（A6）。α_i を超える AC が計上された場合、R-T3 が過負荷として検知する。**α_i = 0 は定義域外**——プロジェクトに時間を投下しない人間は割当解除（§2.4）で表現し、α_i のスコープからは除外する。
**α_i の定義域 (0, 1.0] の根拠:** 1.0MD/日は標準稼働日の容量を単位とする（A6）。残業等による一時的な超過は AC に正直に反映されるが、計画上の容量（α_i）は標準稼働を上限とする——超過労働を計画に恒常的に織り込むことは本モデルの設計哲学（P0: 正直な計測）に反する。
**α_i の記録:** α_i はノードレベルのイベントログ（A2）とは異なる**プロジェクト構成入力**である。実装はα_i の変更履歴を追跡可能に維持しなければならない（R-U14）。α_i の変更は P7 スケジュール再導出をトリガーする。
**複数プロジェクトへのまたがり:** α_i は単一プロジェクト視点で定義される。複数プロジェクトにわたる Σα_i の整合性は本モデルのスコープ外であり、組織レベルのリソース管理の責務である。
*The only scarce resource is humans. Each human has an allocation rate **α_i ∈ (0, 1.0] MD/day** — a human-provided input (§2.1 #5); default 1.0MD/day (backward compatible). α_i is the fraction of time available to this project, NOT capability. The system does NOT model human capability — it is growth-oriented and run by human flexibility (mentoring), not hard rules.*
*α_i is a planning capacity cap; it does NOT constrain AC (actual cost) recording. AC honestly records attention-time spent (A6). When AC exceeds α_i, R-T3 detects overload. α_i = 0 is excluded — a non-participating human is expressed via unassignment (§2.4). The domain (0, 1.0] reflects standard workday capacity (A6); overtime appears in AC, not in α_i. α_i is a project configuration input (distinct from the node-level event log of A2); implementations must maintain auditable change history (R-U14). Cross-project Σα_i consistency is out of scope.*

**A5 — 三つの(非)対称.** 人間とエージェントは、**表現において対称**(同一ログ・同一の提案者役割)、**資源において非対称**(各人間の α_i を上限とする平準化対象は人間のみ、エージェントは遊休可)、**権限において非対称**(見積の確定=合意は人間のみ)。
*Symmetric in representation; asymmetric in resource (only humans leveled, each at their α_i; agents idle-tolerant) and authority (only humans finalize estimates).*
...
> **導出を調整する裁量パラメータは存在しない。** 重み・留保率・ready 閾値・スキルは、見積データ・ノード化・辺ごとのポリシー・人間判断へ還元され消滅した。α_i（各人間の割当率; A4）は裁量的な調整値ではなく、雇用契約・組織的アサインメントに基づく**外的制約**の入力であり、ここで廃止されたパラメータ群には含まれない。
> *No discretionary tuning parameters. Weights, reserve, ready-threshold, and skills all dissolved into estimates, node structure, per-edge policy, and human judgment. α_i (each human's allocation rate; A4) is NOT a tuning parameter — it is an external constraint input based on employment/organizational assignment, and is not among the dissolved knobs.*
...
5. **人間ごとの割当率(α_i)** — 各人間が当該プロジェクトに投下可能な容量は人間が宣言する(A4)。
...
スケジュールは、人間が与えた割当のもとでの **α_i 平準化**（各人間の割当率を上限とする資源制約）として導出。エージェント作業は平準化対象外でスラックを持たず、通常は人間接点の間隙を埋める。**ただしエージェントタスクが依存連鎖上で人間タスクを律速する場合(A→H)、A のリードタイム(P6)はクリティカルパス計算に算入される**——クリティカルパスは全依存連鎖のうち最長のパスであり、人間タスクは α_i 平準化（資源制約）の対象、エージェントタスクは非平準化だがリードタイム(P6)でパス長に寄与する。
*Schedule = **α_i leveling** (resource-constrained to each human's allocation rate) over human-given assignments; agent work fills gaps. However, when an agent task rate-limits a human task on the dependency chain (A→H), A's lead time (P6) enters the critical path — the critical path is the longest path through the full DAG — human tasks are α_i-leveled (resource-constrained), agent tasks are not leveled but contribute their lead time (P6) to path length.*
...
**R-U11.** The system shall subject only human resources to **α_i MD/day leveling** (each human's allocation rate; default 1.0MD/day when unspecified) and leave agent resources unconstrained.
本システムは **α_i MD/日 平準化**（各人間の割当率; 未指定時 1.0MD/日）を人間資源にのみ適用し、エージェント資源を制約対象外としなければならない。

**R-U12 (Unwanted).** If two or more **distinct** human actors emit `proposed→agreed` transitions on the same estimate node with **differing frozen values**, then the system shall raise a **contradiction-agreement warning** identifying the conflicting actors and values. Detection is actor-based: different actors + different values = warning, regardless of temporal proximity. The system shall not auto-resolve; the human decides (P0). The latest-wins value (I3) remains the current value pending human resolution.
二人以上の**異なる**人間が同一見積ノードに**異なる凍結値**で `proposed→agreed` を発行した場合、本システムは**矛盾合意警告**を発し、競合する行為者と値を特定しなければならない。検知は行為者ベース:異なる行為者 + 異なる値 = 警告(時間的近接性は問わない)。自動解決はせず人間が決定する(P0)。人間の解決を待つ間、I3 latest-wins の値が現行値として扱われる。
...
**R-U14 (Ubiquitous).** The system shall maintain an auditable change history of each human's allocation rate α_i, enabling reproducible schedule derivation (P7). α_i is a project configuration input distinct from the node-level event log (A2); its change history need not conform to the four event types but must be append-only and timestamped.
本システムは各人間の割当率 α_i の変更履歴を追跡可能に維持し、スケジュール導出（P7）の再現性を確保しなければならない。α_i はノードレベルのイベントログ（A2）とは異なるプロジェクト構成入力であり、その変更履歴は 4 イベント種別に準拠する必要はないが、追記専用かつタイムスタンプ付きでなければならない。
...
**R-S7 (State).** While a sub-unit's current assignment differs from the assignment at slot-freeze time, the system shall flag the baseline slot as potentially stale and surface the stale-slot count as a visible gap alongside schedule coverage (R-S6), prompting the human to re-baseline or confirm.
ベースライン・スロット凍結時の割当と現行割当が異なるサブ単位について、本システムはベースライン・スロットを陳腐化の可能性ありとして標識し、陳腐化スロット数をスケジュール・カバレッジ(R-S6)と並ぶ可視のギャップとして提示し、人間に再ベースラインまたは確認を促さなければならない。(R-S7 と R-S6 は独立した可視のギャップである:R-S6 はスケジュール・カバレッジの低さで SPI をデレートし、R-S7 は陳腐化スロットを個別に標識する。両方が同時に該当しうる。)

### 4.3 スケジュール・期日・割当 / Schedule, Deadline, Assignment

**R-T1 (Ubiquitous).** The system shall derive the schedule as an **α_i-leveled**, critical-path-priority heuristic over the human-provided assignments and the DAG, recomputed incrementally on each event.
本システムはスケジュールを、人間が与えた割当と DAG の上での **α_i 平準化**・クリティカルパス優先の発見的解として導出し、イベント毎に増分再計算しなければならない（増分再計算は発見的・実行可能解の維持であって、毎回の大域最適化を要求しない; P8）。
...
**R-T3 (Unwanted).** If the human-provided assignment cannot fit **α_i MD leveling** for some person over some window, then the system shall raise an over-allocation alert identifying the person and window for the human to rebalance.
人間が与えた割当が、ある人物・ある期間で **α_i 平準化**に収まらない場合、本システムは当該人物と期間を特定する過負荷アラートを発し、人間が再調整できるようにしなければならない。
...
導出スケジュールが外部期日を超える場合、本システムはアラートのみを行い、要員追加やスコープ削除を自動で行ってはならない。判断は人間が行う。(導出スケジュールはエージェント区間のリードタイムを含む(R-T2)。これにより、エージェント作業が人間を律速する場合も期日超過アラートが正しく発火する。)
...
**α_i の必要性（v12 で追加）:** α_i（人間ごとの割当率）は v11 までの固定容量 1.0MD/日 に対し、部分参加チーム（0.2MD/日〜1.0MD/日 混在）という新 UC を正直に扱うために導入された。α_i なしでは P7 が全員 1.0MD/日 前提の楽観的スケジュールを導出し、R-T3 の過負荷検知が部分参加者に対して**偽陰性**を生む。P0（人間の運用判断）で吸収可能だが、その場合システムの導出指標（スケジュール・SPI）が**意図的に不正確になる**——これは P0「コミット領域のみを正直に語る」の精神に反する。α_i はノードレベルの 4 イベントには載らないプロジェクト構成入力であり、A2 の射程（ノードのワークデータ）とは別次元で存在する（R-U14 で追跡を要求）。v5 で廃止した裁量パラメータ（重み・留保率・スキル）は導出を恣意的に調整する値だったが、α_i は雇用契約・組織的アサインメントに基づく外的事実の入力であり、質的に異なる。
*α_i necessity (added in v12): α_i was introduced to honestly handle fractional-allocation teams — a new UC where people contribute 0.2–1.0 MD/day. Without α_i, P7 produces optimistic schedules (1.0MD assumption) and R-T3 yields false negatives for part-timers. P0 can absorb this operationally, but then the system's derived metrics become intentionally inaccurate — contrary to P0's "speak honestly of the committed region." α_i is a project configuration input outside the scope of A2's node-level event log (tracked via R-U14). Unlike the dissolved tuning parameters (weights, reserves, skills) which were discretionary adjustments, α_i is an external fact based on employment/organizational assignment.*

**意図的に持たないもの:**
- 審議過程 → `cause` が指す spec 文書の責務。
- 重み/留保率/能力(スキル・習熟度)→ 全廃（α_i は外的制約入力であり本項の廃止対象ではない。上記参照）。
- 四つの判断（見積合意・割当・スコープ/期日・α_i 設定）の自動化 → 持たない(P0/§2.1)。
- 未発見作業の測定 → 原理的に不能(P2)。
- 人間内部の権威・オーナーシップ → A4 の延長としてスコープ外(§2.1)。矛盾合意は警告のみ(R-U12)。
- ペアリング/モブプログラミングの構造的表現 → 消費する UC が無く最小性に反するため持たない(§2.4)。
- 複数プロジェクトにわたる Σα_i の整合性 → 本モデルは単一プロジェクト視点であり、クロスプロジェクトのリソース整合は組織レベル管理の責務(A4)。
...
「完全に閉じた」という宣言はしない(本モデル自身の P2「未発見は測れない」と一貫させる)。v12 確定までに v11 の残余に加え、α_i パラメータ化で Critical 5 件を決着させた。以下は v12 時点で認識している残余の開示:
...
4. **α_i の陳腐化検知**(v12):α_i はプロジェクト構成入力であり、実態と乖離しうる。R-S7（ベースラインスロット陳腐化）と異なり、α_i の陳腐化を検知する構造的トリガー条件が存在しないため、専用の警告要件は設けていない。陳腐化の認識と更新は P0（人間の運用判断）+ R-U14（変更追跡）に委ねる。

構造・語彙・既定値は実用上安定しており、次の段階として本モデルを前提とした具体的アーキテクチャの検討に進める。今後さらなる綻びが見つかれば、本書は moira-model-update スキルを通して追記的に更新される(追記専用の精神)。
*We do not declare the model "fully closed" (consistent with the model's own P2). As of v12, all Critical/Important findings from thought experiments 04–10 and the α_i parameterization are resolved. Residual disclosures: (1) frozen-slot selection is implementation-dependent; (2) CPI is modeling-choice-dependent; (3) granularity asymmetry; (4) α_i staleness has no structural trigger — detection relies on P0 + R-U14. Structure, vocabulary, and defaults are practically stable; the next stage is concrete architecture.*
