# 仕様駆動・チケット駆動・EVM 統合モデル v13
# Unified Model: Spec-Driven × Ticket-Driven × EVM (v13)

> 合言葉 / Motto: **必要かつ十分 (Necessary and Sufficient)**
>
> 本書はアーキテクチャ以前の「思想の確定」であり、実装技術には立ち入らない。
> v13 は、祝日・休暇・部分稼働日を正直に time-phased PV へ反映するため、A4 の割当率 α_i（スカラー）を**人間ごとの日次容量 c(i,d) ∈ [0, 1.0] MD/日**（既定 1.0、理由付き append-only 改定）へ一般化した版である。α_i は廃止ではなく **c の reason=契約 成分の導出 view** へ降格（契約レートの意味は保持）。容量を単一の per-date 入力に一本化した（α×暦 の二因子分離は、どの導出も積しか読まず冗長・かつ圧縮勤務で破綻するため不採用）。主な修正: A4(c 一般化・α_i を導出 view 化・c は時間可用性で能力でない・c=0 込み・slot 次元のみ)、A5/L45/§2.1#5(α_i→c)、P7/R-U11/R-T3(α_i 平準化→c 平準化)、R-U14(c 履歴追跡)、R-S7(陳腐化トリガーを「予測終端の乖離」へ一般化＝cause-agnostic)、§3(c は slot に効き予算に効かない)、§5(c 必要性・最小性・2層データ・スコープ外)、§7(drift 非検知・暦既定・洪水・再現性を開示)。新公理・新イベント・新状態・新**要件番号**は増やさないが、機械的な一般化に留まらない: R-S7 は発火条件の意味論拡張（cause-agnostic 化）、R-T3 は新発火事象（c=0 日の AC）を得、v12 §7#4 の開示を撤回する。
> *v13 generalizes v12's scalar allocation rate α_i into a **per-human daily capacity c(i,d) ∈ [0, 1.0] MD/day** (default 1.0, reason-stamped append-only revisions), so that holidays, leave, and partial-availability days are honestly time-phased into PV. α_i is not removed but demoted to a **derived view** (the reason=contract component of c), preserving the contract-rate meaning. Capacity is unified into a single per-date input (the rate × calendar two-factor split is rejected: no derivation reads the factors apart — only the product — and it breaks on compressed schedules). Key changes: A4 (c generalization, α_i as derived view, c is time-availability not capability, c=0 included, slot dimension only), A5/L45/§2.1#5 (α_i→c), P7/R-U11/R-T3 (α_i→c leveling), R-U14 (c history), R-S7 (staleness trigger generalized to forecast-end divergence = cause-agnostic), §3 (c affects slot not budget), §5 (c necessity, minimality, two-tier data, scope-outs), §7 (drift undetectability, calendar default, flood, reproducibility disclosed). No new axioms, events, states, or requirement-numbers; but beyond mechanical generalization: R-S7 gains a semantic extension (cause-agnostic), R-T3 a new firing event (AC on a c=0 day), and v12's §7#4 disclosure is retracted.*
...
人間が与えた割当が、ある人物・ある期間で **c 平準化**に収まらない場合（c=0 の日に AC が計上された場合を含む）、本システムは当該人物と期間を特定する過負荷アラートを発し、人間が再調整できるようにしなければならない（発火閾値・期間は実装定義）。
...
「完全に閉じた」という宣言はしない(本モデル自身の P2「未発見は測れない」と一貫させる)。v13 確定までに、α_i スカラーの c(i,d) 一般化で独立敵対者の Critical 群（容量の二因子分離の冗長性・圧縮勤務での破綻・cause-agnostic R-S7 と旧開示の衝突 等）を決着させた（中核分岐＝単一容量 c か二因子か、はユーザー裁定で単一 c）。以下は v13 時点で認識している残余の開示:
