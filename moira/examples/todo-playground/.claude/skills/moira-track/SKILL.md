...
> **このスキルの正体**: 背骨フロー [`new-feature-happy-path`](../../../../../.kiro/scenarios/flows/new-feature-happy-path.md) §5 と
> [ADR-0001](../../../../../.kiro/adr/0001-moira-cli-write-path.md) が名指す **`moira-progress` 継ぎ目（⚠未実装）の MVP 実体**。
...
| 層 | 置き場所 | 役割 |
|---|---|---|
| **いつ発火するか** | playground `CLAUDE.md` の発火トリガー表（常時ロード） | 節目の取りこぼし検知 |
...
- フィーチャー（spec）= `<feature>`（spec 名。親はプロジェクト根 `todo-app`）。
- フェーズ子 = `<feature>/req` `<feature>/design` `<feature>/tasks`。実装 = `<feature>/impl-1…`、実装レビュー = `<feature>/review-impl`。
- `<feature>` = `--feature` 引数。無ければ `.kiro/specs/*/spec.json` の `name`。**複数 spec なら勝手に選ばず `[人間確認]`**。
...
2. フィーチャー＋3フェーズ子を**未見積**で誕生（`--actor agent:claude`）:
   - `moira add <feature> --parent todo-app --label "<日本語>" --actor agent:claude`
...
- **過不足なく**: 指定フェーズに対応するイベントだけを emit する。フェーズ跨ぎの先回り emit はしない。
...
- `.moira/` 不在: `moira init` を先に（discovery フェーズは自動で行う）。
