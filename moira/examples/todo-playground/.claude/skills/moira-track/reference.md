...
復旧手順・背骨弧の対応・moira 本体側の既知制約**を engine ソースに接地して収めた参照。

> すべての行番号は `moira/backend` `moira/cli`（この checkout）の参照実装に対する接地。**moira 本体は
> 変更しない**（[ADR-0001](../../../../../.kiro/adr/0001-moira-cli-write-path.md)）。ここは「なぜその手順か」の根拠置き場。

---

## §A ノード ID・フィーチャー解決

- **プロジェクト根** = `todo-app`（`.moira/config.json` の `projectRoot`）。フィーチャーの親。
- **フィーチャー（spec）ノード** = `<feature>`（spec 名。例 `task-add-and-list`）。親は `todo-app`。
...
- **`<feature>` の決定**:
  ...
  2. 無ければ `.kiro/specs/*/spec.json` を Glob/Read し、`name` を採る。
...
- 前提: `.moira/` 初期化済み（未なら `moira init --me <あなた> --root todo-app`）。
- 発行（**すべて `--actor agent:claude`・すべて `--parent` 必須**）:
  1. `moira add <feature> --parent todo-app --label "<日本語>" --actor agent:claude`
...
- `--parent` 省略時、親は **プロジェクト根 `todo-app` にフォールバック**する（`commands.ts:116`：`str(values.parent) ?? cfg.projectRoot`）。
- 既存フェーズノードへ見積を後付けする**再 add でも起きる** → `todo-app → <feature>/<phase>` の二重 decompose 辺が生まれ、
...
# 4) 検証: 二重辺が消えたか / メトリクスが元どおりか
grep '"parent": "todo-app"' .moira/events.json   # feature ノード自身の1件のみが正
...
背骨フロー [`new-feature-happy-path`](../../../../../.kiro/scenarios/flows/new-feature-happy-path.md) は **弧の“形”**を定める正典。
