...
- **併記(原文＋訳文)は spec の言語の本文を採る**(#89): 受け入れ基準・目的・タイトル等が原文と訳文の併記(例: 英文の番号付き基準の直後に `- 和訳: …` のぶら下がり行で訳を添える形式)になっている場合、`text`/`objective`/`title` には spec の `language` に一致する言語の本文**のみ**を採る(`language: "ja"` なら和訳側)。原文(他言語)は出力に含めない。併記でない単一言語ならその本文をそのまま採る。これは決定的パーサ(#28)の併記対応と表示意味を一致させるため(consumer はどちらの経路でも同じ和文を表示する)。
