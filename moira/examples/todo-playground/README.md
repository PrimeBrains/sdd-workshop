...
このフォルダのアダプタ一式（`moira-track` スキル・hooks・steering 発火表）は
**`moira adapter install` の設置産物**（インストール済み・`moira adapter status` で検査可能）。
任意の cc-sdd プロジェクトに同じものを入れるには、そのルートで `moira adapter install` を実行する
（正本は `moira/cli/templates/`・設計は [ADR-0002](../../../.kiro/adr/0002-cc-sdd-moira-adapter.md)）。
