...
flowchart TB
    ...
    DS1 --> P3[P3 ルーティング裁定]
    P3 -.->|人間断面ビュー＋判断事項一覧| HA{{HA 人間: 前半集約セッション<br/>マップ確認・境界裁定<br/>When/Then 発案・M/D/P 意図批准<br/>実行計画承認}}
    HA -.-> P3

    subgraph P4 ["P4 ゲート実行"]
        GM["M級: moira-model-update<br/>（NAMING・moira 系 skill/agent 定義含む）"]
        GD["D級: DECISIONS-CATALOG 起票<br/>(a)/混合 → doc-refine + 意図整合検査<br/>(b) → doc-refine 技術ゲートのみ"]
        GP["P級: プロパティ一文（敵対ゲート + 意図整合検査）<br/>PBT 実装（著者分離 + mutation + witness）<br/>生成器/shrinker（一級レビュー）"]
    ...
    GM & GD & GP & GS -.->|"意図からの逸脱 / genuine fork<br/>（ブロック時点までバッチ）"| HB{{HB 人間: fork バッチ}}
    HB -.->|裁定を反映| P3
    ...
    P6 -.-> H5{{H5 人間: クローズ承認（薄い）<br/>3面最終文の意図突合<br/>できないこと差分・閉包一行}}
...
| **P2 影響調査** | 変更要求票＋正典群＋トレース機構（下記）＋（再入時）未マップ差分 | 波及先成果物を列挙。**各行に「クラス・根拠・担当ゲート・期待 postcondition・検証器」を必須記載** | **影響マップ**（作業台帳・append-only）＋**人間断面ビュー**（人間タッチポイントを持つ行＝3面行だけのフィルタ。機械決着行は折り畳み） | AI（調査 subagent 派遣可） |
