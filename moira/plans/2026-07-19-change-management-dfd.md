...
> **状態**: **ratified v5**（2026-07-19 人間批准。codex レビュー第1・第2ラウンド反映済・#40 裁定反映済・
> 人間タッチポイント再設計〔前半集約〕反映済・§8 全件裁定済み。以後の変更は #39 構築の実装知見に基づく
> 軽微な追随を除き、変更管理の対象とする）
...
- **スコープの正直な宣言**: 本フローは「リポジトリへの全変更を強制的に横取りする」機構では**ない**。既存 skill の
  直接起動・直接 commit は技術的に可能なまま残る（**恒久の設計として強制しない**——§8-5 裁定済み。漏れは
  検知器が拾って issue 再入する）。本フローが
...
| 工程 | in | 処理 | out | 主担当 |
|---|---|---|---|---|
| **P1 受付・正規化** | 入口 3 種（issue／検出結果／会話依頼） | issue に正規化し、**人間の言葉（曖昧さ・省略を含む）を後段の LLM 処理が迷わない明確な変更要求文に書き起こす**（原文は issue に残る——情報は失わない）。候補クラス（§3）を仮判定。**triage**（§8-3）: フル工程を回すか、軽量ゆえフローを起動せず通常作業へ送り出すかを判定し、判定理由を issue に一言記録。**受付時点の commit を base として変更要求票に記録**（以後の全差分検査は `base..HEAD`） | 変更要求票（issue 番号・明確化した要求文・候補クラス・triage 判定・**base commit**） | AI（メイン文脈） |
...
| ID | 位置 | 内容 | 性質 |
|---|---|---|---|
| **HA 前半集約セッション** | P3 直後・ゲート実行前（原則 issue につき 1 回） | まとめて一括で諮る: ①影響マップ人間断面の確認（「はねる先はこれで全部か」）②境界裁定（旧 H1）③**When/Then の発案**（旧 H3——**新規シナリオのみ**。既存 unit の変更は issue に人間が書いた記述を発案として扱う——issue に無いときだけここで聞く）④**M/D/P 各行の意図批准**（旧 H2 の方向づけ・旧 H4——何を決めるか・受け入れ基準・却下したい方向を実質で yes/no。仕分け規律は従前: D は (a)/混合のみ・P は一文のみ。**M 行は MODEL 条項文でなく D/P/S 粒度の平易文に翻訳して諮る**——難しすぎる条項文レビューを人間に求めない）⑤実行計画の承認 | 旧 H1〜H4 を**集約** |
...
- **実装帰結（正直開示）**: `kiro-scenario`／`kiro-scenario-flow`／`doc-refine`／`moira-model-update` は
  ...
  通す）。あわせて `kiro-scenario` の「AI は When/Then を発案しない」を「**既存 unit の変更では issue
  記載の人間の言葉を発案として扱ってよい**（新規は従来どおり人間発案）」へ緩和し、`moira-model-update`
  の FORK/批准提示に「**D/P/S 粒度の平易文へ翻訳して諮る**」規約を追加する。本フロー構築時の作業項目とする。
...
## 8. 未決事項 → 全件裁定済み（2026-07-19 ユーザー裁定）

1. **P5 掃射の範囲** → **関係分のみ**: `decision-conformance`／`doc-fact-checker` は「影響マップ範囲＋
   逆引きヒット分」に限定。全量掃射は定期健診として必要になれば別途（今回は組まない）。
2. **影響マップ・閉包レポートの置き場** → `moira/changes/issue-N/` の md として repo に置き、残す。
   非正典マーキング＋issue には人間向け要約のみ＋未マップ差分検査から自己除外（§5・§7）。
3. **軽量パスの要否** → **P1 受付が triage する**（kiro-discovery の action-path 方式と同型）: 受付が
   「フル工程を回すか、軽量ゆえフローを起動せず通常作業に送り出すか」を判定し、判定を issue に一言
   記録する。誤判定の網は既存検知器（DRIFTED・CI・E2E）が引き受ける（§2 P1 に反映）。
4. **陰性対照の恒常化** → **受け入れテスト時のみ**。定期化は運用データを見てから再検討。
5. **bypass 防止の強制** → **強制しない（紳士協定のまま）**: フローを通らない直接変更も普通にアリ。
   §0 のスコープ宣言（「通った変更については保証する」）が恒久の姿となる。漏れは検知器（DRIFTED 等）
   が拾い、issue として再入する。
6. **裏面 ref の正規化** → **正規化ではなく裏面の削除方向**: 手で維持する対応表はドリフト面（R/D/T と
   同型）なので、機械可読化でなく**裏面（根拠 clause・関連実装 ref）自体を削る**。ただし裏面は
   `decision-conformance`（計器⑥）が照合の起点に使うため、削除は「照合の対象特定を AI の意味突合へ
   置き換える」checker 改修とセット。カタログ改訂＋checker 改修を**別 issue として起票**し、本フロー
   完成後の実戦例として流す（§5 の逆引き記述はその issue の確定後に更新）。
...
- **未実施の波及**: 各ゲート系スキルの「ループ内 AskUserQuestion」契約を「事前批准記録の証跡受け入れ」
  ...
  本フロー構築時の作業項目として残る。
- **追加裁定（同日・レビュー対話）**: ①S 級の When/Then 人間発案は**新規シナリオのみ**——既存 unit の
  変更は issue 記載の人間の言葉を発案として扱う（issue に無いときだけ前半セッションで聞く）。
  ②M 級の意図批准は **MODEL 条項文の直接レビューを求めず、D/P/S 粒度の平易文（判定文・一文・
  ふるまい例）へ翻訳して諮る**（3面の平易言語規約の M 級への適用）。§3・§6 に反映済み。
