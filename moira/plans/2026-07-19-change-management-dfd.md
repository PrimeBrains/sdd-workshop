# Moira 変更管理フロー DFD（ドラフト v4）

> **状態**: draft v4（codex レビュー第1・第2ラウンド反映済・#40 裁定反映済・人間未レビュー）
...
> moira-verification 改訂）を反映し **R 級と H6 を廃止**（§11）。
...
flowchart TB
    ...
    IN1 --> P1[P1 受付・正規化]
    P1 -->|変更要求票| P2[P2 影響調査]
    P2 --> DS1[(影響マップ<br/>行 = 波及先 + クラス + 根拠<br/>+ 期待 postcondition + 検証器)]
    DS1 --> P3[P3 ルーティング裁定]
    P3 -.->|境界不明瞭| H1{{H1 人間: 境界裁定}}
    H1 -.-> P3

    subgraph P4 ["P4 ゲート実行"]
        GM["M級: moira-model-update<br/>（NAMING・moira 系 skill/agent 定義含む）"]
        GD["D級: DECISIONS-CATALOG 起票<br/>(a)/混合 → doc-refine + H4 批准<br/>(b) → doc-refine 技術ゲートのみ"]
        GP["P級: プロパティ一文（敵対ゲート + H4）<br/>PBT 実装（著者分離 + mutation + witness）<br/>生成器/shrinker（一級レビュー）"]
        GS["S級: kiro-scenario（unit）<br/>kiro-scenario-flow（flow）<br/>unit agreed → kiro-scenario-e2e<br/>（flow-level E2E は未整備）"]
        GC["C級: kiro-impl<br/>（開始時に必要なら R/D/T を<br/>正典から再生成——使い捨て）<br/>+ codex + CI"]
        GV["V級: 検証基盤・CI<br/>（独立レビュー + gate inventory 比較）"]
        GF["F級: 一般確定文書<br/>doc-refine"]
    end
    ...
    GM -.->|"正典確定後: P3 再裁定<br/>（影響マップの下流行を再実行計画へ）"| P3
    GP -.->|上流確定後| GC
    GS -.->|上流確定後| GC

    GD -->|"agreed が MODEL の制約・語彙・既定に触れる → 昇格"| GM
    GS -->|"MODEL 矛盾の上申"| GM
    GS -->|"flow のメンバー欠陥差し戻し"| GS

    GM -.-> H2{{H2 人間: MODEL 裁定}}
    GS -.-> H3{{H3 人間: When/Then 発案}}
    GD -.-> H4{{H4 人間: 批准}}
    GP -.-> H4
    GS -.-> H4

    GM & GD & GP & GS & GC & GV & GF --> ART[(変更成果物<br/>= コミット群)]
    GM & GD & GP & GS & GC & GV & GF --> DS2[(ゲート証跡<br/>批准記録 / CI 結果 /<br/>照合 verdict / レビュー結果)]

    ART -->|"各ゲート後: diff(base..HEAD) の<br/>changed − mapped ≠ ∅ なら<br/>未マップ path 集合を渡す"| P2

    DS1 --> P5[P5 同期閉包確認<br/>diff = base..判定時 HEAD を固定]
    DS2 --> P5
    ART -->|git diff| P5
    P5 -->|"未了行あり / 未マップ差分あり"| P2
    P5 -->|閉包レポート| P6[P6 クローズ]
    P6 -.-> H5{{H5 人間: クローズ承認}}
    P6 --> DONE([issue クローズ<br/>+ 該当する確定分岐がある場合のみ<br/>ジャーナル来歴記録])
```

凡例: 実線＝データフロー（変更要求票・影響マップ行・成果物・証跡）、点線＝制御・順序・人間タッチポイント。

## 2. 各工程の in/out

| 工程 | in | 処理 | out | 主担当 |
|---|---|---|---|---|
| **P1 受付・正規化** | 入口 3 種（issue／検出結果／会話依頼） | issue に正規化し、変更要求として要約、候補クラス（§3）を仮判定。**受付時点の commit を base として変更要求票に記録**（以後の全差分検査は `base..HEAD`） | 変更要求票（issue 番号・要約・候補クラス・**base commit**） | AI（メイン文脈） |
| **P2 影響調査** | 変更要求票＋正典群＋トレース機構（下記）＋（再入時）未マップ差分 | 波及先成果物を列挙。**各行に「クラス・根拠・担当ゲート・期待 postcondition・検証器」を必須記載** | **影響マップ**（作業台帳・append-only） | AI（調査 subagent 派遣可） |
| **P3 ルーティング裁定** | 影響マップ | §3 の規則で経路と依存順を確定。境界不明瞭なら **H1 上申** | 実行計画（経路列・依存順・人間タッチポイント予定） | AI＋人間（H1） |
...
| **P5 同期閉包確認** | 影響マップ＋ゲート証跡＋実差分。**差分の両端を固定**: base＝P1 で記録した変更開始前 commit、対象＝P5 開始時の HEAD。**照合中に HEAD が動いたら判定無効・再実行** | 影響マップの各行を resolved／deferred／未了に判定（§5）。`diff(base..HEAD)` に対する未マップ差分の非存在も確認 | **閉包レポート** | AI（照合 subagent） |
| **P6 クローズ** | 閉包レポート | 人間承認（H5）→ issue クローズコメント。ジャーナル来歴は**該当する確定分岐がある場合のみ**（記録条件は `moira-model-update`／目録の規律に従う——本フローは条件を新設しない） | クローズ済み issue | AI＋人間（H5） |
...
|---|---|---|---|
| **M（MODEL・正典設計物級）** | MODEL の公理・制約・語彙・既定・イベント意味論に**積極的に矛盾または変更**。**および `moira-model-update` が所有する確定設計物**（NAMING・命名文書・同 skill 自身・moira 系 agent 定義） | `moira-model-update`（敵対ゲート＋同一 run での bound プロパティ再批准＋派生文書同期） | H2 |
| **D（設計判断級）** | MODEL がその論点に**沈黙**している 0→1 の構造判断（境界・所有権・依存方向・enforce の所在）。沈黙は「衝突」でなく「拡張」——Decision は中間状態として有効 | DECISIONS-CATALOG に `proposed` 起票 → `doc-refine` → **仕分け弁**: (a) 責任・帰結・ドメイン意味＋混合 → H4 批准／(b) 純工学 → doc-refine 技術ゲートで確定（PO レビュー対象にしない）。**agreed 後、MODEL の制約・語彙・既定に触れるなら M へ昇格**（D → M → 下流再実行のループ。§4） | H4（(a)/混合のみ） |
| **P（プロパティ級）** | 不変条件の新設・改訂。**3 サブルートに分岐**: ①プロパティ一文（散文と同じ敵対ゲート＋H4 批准）②PBT 実装（**著者≠実装者**・mutation で非空虚性・到達 witness・決定的 seed・CI 実走）③**生成器/shrinker**（正典上「最大のリスク・一級レビュー対象」——敵対的構成レビュー＋自己被覆アサーション） | PROPERTIES.md＋PBT 資産（各サブルートの義務は左記） | H4（一文のみ） |
| **S（シナリオ級）** | 観測ふるまい・外的妥当性。**unit と flow は別ルート**: unit → `kiro-scenario`（**人間が When/Then を発案**——AI は §3 を発案しない）／flow（`.kiro/scenarios/flows/`）→ `kiro-scenario-flow`（合成範囲は人間承認・メンバー欠陥は unit へ差し戻し）。**計器③（`kiro-scenario-e2e`）再生成は agreed unit のみ**——flow-level E2E 自動化は `kiro-scenario-flow` の規範どおり未整備（follow-up）であり、flow 行の検証器は flow 文書自体の確定ゲート（checker＋敵対ゲート）まで。flow は構成 unit の per-unit E2E で間接的に覆われるに留まる（正直開示） | 上記 skill 群 | H3（発案）＋H4（批准） |
...
（D → M）が拾う。既存 MODEL 条項と**積極的に矛盾**することが最初から明白な場合のみ直接 M。それでも迷うなら H1。
...
加えて**未マップ差分の非存在**（`diff(base..HEAD)` の changed − mapped = ∅。base＝P1 記録の変更開始前 commit、
...
|---|---|---|
| MODEL 変更の派生同期（NAMING・steering・ジャーナル・参照実装 types/fold） | `moira-model-update` ゲート自身の同期義務＋`doc-fact-checker`（影響マップ範囲） | ゲートの「残存 Critical=0＋Important 全件 disposition」判定＋批准記録（deferred Important があれば閉包レポートに全件列挙し、その後続 issue の生存確認は H5 が担う）。**M 行の postcondition は `moira-model-update` の現行規範が実施する範囲**（敵対ゲート・bound プロパティ再批准・派生同期）に限る——**境界モデル検査（第2器）は未実装（正典自身が「到達目標」と開示）のため resolved 条件に含めず**、その不在は下記「P5 の限界」の未整備義務に計上する |
...
| ID | 位置 | 内容 | 性質 |
|---|---|---|---|
| **H1** | P3 | 境界不明瞭 issue の分類裁定 | 本フロー**新設** |
| **H2** | P4-M | `moira-model-update` 内の FORK/ユーザールーティング | 既存 |
| **H3** | P4-S | シナリオ When/Then の発案（レビューでなく**入力**。0→1 は人間） | 既存 |
| **H4** | P4-D/P/S | proposed→agreed 批准。**適用範囲は仕分け規律に従う**: D は (a)/混合のみ（(b) 純工学は技術ゲートで確定・PO 対象外）、P はプロパティ一文のみ、S は unit/flow 批准 | 既存 |
| **H5** | P6 | 閉包レポートの承認と issue クローズ（deferred 行の後続 issue が生きていることの確認を含む） | 本フロー**新設** |

新設は **H1 と H5 の 2 点のみ**。「人間の関与はこの表の点に限定される」が本フローの主張であり、点数の固定
（v1「5点」）ではなく**表への明示**が本質。（v3 まで存在した H6〔移行期 R/D/T レビュー〕は #40 の R/D/T
使い捨て化により**廃止**——§11。）
