...
- `AskUserQuestion` で **ユニット集合・順序・起点/終点・スコープ外の線引きを人間に明示承認させ、その確定記録を残す**（これがゲートの `SCOPE_APPROVED` の根拠）。承認されるまで先に進まない。
  **変更管理フロー（`moira-change` P4）起動時**: HA 事前批准記録がこの線引き（ユニット集合・順序・起点/終点・スコープ外）を実質的に確定しているなら、その記録を `SCOPE_APPROVED` の根拠として受け入れてよい（`../doc-refine/review-gate.md`「変更管理フロー連携」。覆っていなければ従来どおり諮る——フローでは HB バッチへ）。
...
- **変更管理フロー（`moira-change` P4）起動時**: HA 事前批准記録が通し価値の**意図＋受け入れ基準**を与えているなら、その記録を `DRAFT_RATIFIED` の証跡として受け入れてよい（最終文の逐語批准は採点者の**意図整合検査**が代替——逸脱検出時は PASS を出さずフローの HB へ上申。`../doc-refine/review-gate.md`「変更管理フロー連携」）。批准記録が通し価値を覆っていなければ従来どおりこの段で批准を取る。
  **正直開示（リスク集中点）**: フローの通し価値は AI 生成度が最も高く、HA 時点では生成物がまだ存在しない——事前批准が覆うのは意図＋受け入れ基準であり、生成後の通し価値本文を人間が読むのは H5 の抜き取り突合のみ。よって flow 行は閉包レポートの意図突合対応表で**明示的に区別**し、H5 で優先的に確認できるようにする。
...
- **5b 分岐のユーザールーティング:** 敵対者が `FORK` とした、または分岐と認めた**人間の意図に属する判断**（特に FC4 の通し価値の方向・スコープの線引き）は、著者が勝手に決めず `AskUserQuestion` でユーザーに回す。**変更管理フロー起動時**は `../doc-refine/review-gate.md`「変更管理フロー連携」に従う: HA 事前批准記録が分岐を実質的に決めているなら記録で決着とし、覆わない genuine fork はフローの fork バッチ（HB）へ回す（裁定を得る前に確定はできない）。採点（5f）では批准記録を採点者に渡し、意図整合検査を判定に含めさせる。
...
- **5e パッチ（著者のみ）:** 指摘・訂正・ユーザー決定を反映して**主コンテキストだけ**がフロー文書をその場編集する。
  ...
  **変更管理フロー起動時の例外**: §4 への即時差し戻しはせず、当該改変を**批准済み意図からの逸脱**としてフローの fork バッチ（HB）へ登録し、裁定まで本フローの確定を保留（AWAITING-HB）する（即時割込みを避ける DFD §6 の設計。二重経路にしない——フロー起動時の逸脱裁定は常に HB）。
- **5f 採点:** `doc-gate-judge` を1体起動し、`../doc-refine/review-gate.md` 基準で PASS/FAIL を取る。FAIL なら次ラウンド（5a へ）。**例外（変更管理フロー起動時）**: 採点で未決が AWAITING-HB の保留のみなら、次ラウンドの敵対派遣は行わず、HB 裁定を得て反映したのち**同一ラウンドとして採点を再実行**する（ラウンド番号を進めない——`../doc-refine/review-gate.md`「変更管理フロー連携」）。
