...
2. **実行**:
   ...
   **マークダウン保存**（求められたとき／定例で残したいとき）:
   ```bash
   moira report --save-dir <出力ディレクトリ>   # <dir>/moira-report-<projectRoot>-<asOf>.md を生成
   moira report --out <任意のパス.md>            # 任意のファイル名で保存
   ```
   どちらも stdout にも出力し、保存先を stderr の `saved: <path>` で示す。保存先ディレクトリは
   自動作成される。プロジェクト固有の保存先規約があればそれに従う（例: pb-workspace は
   `progress/reports/`）。
