...
**現状は定義済・サンプル試走済（2026-06-27 テスト基盤ステージで D-34／D-40 を試走＝ともに ALIGNED・機構の動作を確認）。網羅実走は後続ステージ。** 採点ゲートの主体は**方向(A)確定・実装は後続**（手順5）。
...
### 3. checker 派遣（並列・独立）
- `decision-conformance-checker` を**並列・独立**に起動（`model: sonnet`（機械的照合であり設計判断をしない worker のため。判断が要る争点は敵対ゲート側）、Write/Edit を与えない）。コスト上限は `model-and-subagent-policy.md` に準ずる（1回の並列は概ね最大3体、対象が多ければバッチで回す）。
...
### 5. ゲート判定（方向(A)確定・実装は後続）
- **採点ゲートに doc-gate-judge は流用不可（確定）。** `doc-refine` の `doc-gate-judge` は **doc-refine 専用基準**（doc-adversary 指摘の決着・doc-fact-checker の SOURCE_SET・FORK ルーティング）で PASS/FAIL を下すため、それらを生成しない conformance には**そのままは流用できない**。
- **方向(A)で確定（2026-06-27・人間合意）**: conformance **専用の薄い採点者を新設する**方向とする——判定基準は「**agreed 判断のうち未ルーティングの DRIFTED = 0**」のみ（`proposed` の drift はブロッカーにしない）。これにより doc-refine 同型の三者分離（著者≠照合者≠採点者）を将来そろえる。**実装は後続ステージ**（本ステージは方向確定のみ）。検討した代替 (B)＝当面採点者を置かず checker の証拠付き出力＋著者集約で代える案は、現に稼働する分離が「著者≠照合者」のみに留まるため、暫定としては成り立つが**恒久形は (A)** とする。
...
| 採点者 | **方向(A)確定＝専用採点者を新設（2026-06-27 合意・実装は後続）** | 「agreed の未ルーティング DRIFTED = 0」の PASS/FAIL。doc-gate-judge は基準不一致ゆえ流用不可 |

現時点で**実機能している分離は「著者 ≠ 照合者」**（checker は独立エージェントで Write/Edit 非保有）。採点脚は**方向(A)確定だが未実装**ゆえ、現状を「doc-refine 同型の三者分離が稼働」と過大に謳わない（採点脚が実装されて初めて三脚が揃う）。
