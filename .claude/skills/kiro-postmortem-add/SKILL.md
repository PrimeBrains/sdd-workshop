...
**Mission**: 不具合 1 件を漏れなく構造化記録し、後段の `/kiro-postmortem-review` が集約分析できる入力として ledger に蓄積する。
...
- Read `.kiro/postmortem/defects.md` (既存 ledger があれば)
  - 存在しない場合は Step 2 で初期化
- Read `.claude/skills/kiro-postmortem-add/rules/taxonomy-reference.md` (4 タクソノミー定義)
...
- Read `.claude/skills/kiro-postmortem-add/templates/ledger-header.md` をテンプレ展開
- Read `.claude/skills/kiro-postmortem-add/templates/seed-entries.md` の 3 件 seed body を取得
...
| 根本要因詳細 | 「なぜそのメカニズムが発動したか」の具体的文章を生成。**撤回条件タグの規律**: 根本要因が「敵対ゲートで追跡付き deferred にした Important の実害化」なら literal タグ `[deferred-important]` を、「R/D/T 使い捨て方針下での維持 spec 不在／再生成物の不忠実」なら literal タグ `[rdt-disposal]` を本文に含める（各ゲート/steering の撤回条件トリガが grep で数えるため。該当しなければ付けない） |
