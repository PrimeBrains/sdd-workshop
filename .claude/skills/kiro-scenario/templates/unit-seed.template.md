<!--
  ...
  ★このファイルでやること: 下の「## 3. ふるまい（When / Then）」の発案（0→1）を、あなた自身が行う。
  ★平易な言葉で自分で書くか、または kiro-scenario に向けて自分の言葉で語れば AI が忠実に転記します
    （AI が 0→1 でふるまいを発案することはしません＝人間所有のガードレール）。書けたら kiro-scenario を再実行してください。
