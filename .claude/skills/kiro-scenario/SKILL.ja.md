...
> **ガードレール（最重要）:** シナリオの**種＝§3 のふるまい（When/Then）は人間が書く。AI は §3 を生成しない。**
> AI の役割は接地・描き起こし・敵対的検証であって、種を蒔くことではない。これを破ると「AI が書いた仕様を
> AI が作ったシナリオで通す」自己採点に退化し、外的妥当性の検証というシナリオの存在意義が失われる。
...
slug を `.kiro/scenarios/units/{slug}.md` に解決する。
- ファイルが**無い**、または **§3「ふるまい（When / Then）」が空/プレースホルダのまま**なら：
  `templates/unit-seed.template.md` を読み、`{{SLUG}}`/`{{TITLE}}` を置換して当該パスに **`Write`（種だけ）し、ここで停止**する。
  ユーザーに「§3 を平易な言葉で自分で書いて再実行してください」と告げる。**§3 に AI は一切書かない。**
- §3 が**人間によって書かれている**なら先へ進む。以降、**§3 は不可侵の人間意図**として扱い、描き起こし・パッチで改変しない。
