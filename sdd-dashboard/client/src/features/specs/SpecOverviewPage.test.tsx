...
describe("SpecOverviewPage（validation レポートの選択提示、Requirement 1.2 / 1.3）", () => {
  it("存在する validation（gap）は /specs/:feature/validation/:type へのリンク、不在（design / impl）は未作成ディムになる", async () => {
    renderOverview();

    const gapTab = await screen.findByTestId("doc-tab-validation-gap");
    expect(gapTab.getAttribute("data-state")).toBe("available");
    expect(gapTab.tagName).toBe("A");
    expect(gapTab.getAttribute("href")).toBe("/specs/fixture-mixed/validation/gap");

    const designTab = screen.getByTestId("doc-tab-validation-design");
    expect(designTab.getAttribute("data-state")).toBe("missing");
    expect(designTab.tagName).not.toBe("A");
    expect(designTab.textContent).toContain("未作成");

    const implTab = screen.getByTestId("doc-tab-validation-impl");
    expect(implTab.getAttribute("data-state")).toBe("missing");
    expect(implTab.textContent).toContain("未作成");
  });

  it("存在する validation タブのクリックで /specs/:feature/validation/gap へ遷移する", async () => {
    const router = renderOverview();

    fireEvent.click(await screen.findByTestId("doc-tab-validation-gap"));
  ...
  it("不在 validation タブのクリックは遷移もエラーも起こさない", async () => {
    const router = renderOverview();

    fireEvent.click(await screen.findByTestId("doc-tab-validation-design"));
