...
- hooks（`moira-guard.mjs` / `moira-fire.mjs`）は同じ解決を**自前の複製ヘルパ**で行う
  （各 .mjs は自己完結・依存ゼロ・fail-open を維持。import で共有しない）。
- 実装: `cli/src/home.ts` `resolveMoiraHome()`（純関数・`home.test.ts` で網羅）。

### Stage 2（計画・未実装）— 宣言的 provider 設定＋汎用エンジン
...
### Stage 3（計画・未実装）— 生成スキル `moira-adapter-gen`
