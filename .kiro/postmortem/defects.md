...
| `sdd-core` | `.kiro/specs/sdd-core/` 配下 (sdd-dashboard アプリ。API・パーサー・共通契約) |
| `sdd-review-ui` | `.kiro/specs/sdd-review-ui/` 配下 (sdd-dashboard アプリ。spec 閲覧 UI) |
| `sdd-workflow-ui` | `.kiro/specs/sdd-workflow-ui/` 配下 (sdd-dashboard アプリ。ワークフロー UI) |
...
### 0004: sdd-dashboard 構造化ビューアが正典スケルトンから逸脱し design 本文破棄 / tasks に誤った amber フォールバック帯

Status: recorded
Entry ID: 0004
Created: 2026-06-12T05:33:58Z
Source: organic

#### 1. 発生機能
`sdd-review-ui / DesignView・TasksView`

#### 2. 発生した不具合
sdd-dashboard 本番クライアントの 2 件の表示不具合。(1) Design ビューが design.md 本文を破棄し、見出し＋Requirements Traceability 表のみ描画していた。`DesignDoc` 契約 (`server/src/types/spec.ts`) に本文を運ぶフィールドが無く、`parseDesign` (`server/src/parsers/design.ts`) が `coverGaps` を呼ばないため、見出し間のプローズ・図表が全て失われる (情報無欠落不変則 13.2/13.3 違反)。正典スケルトン `SpecViewer` の `DesignTab` は TOC＋Traceability 表の下に `<Md>{detail.raw.design}>` で本文を全文描画しているが、本番はこれを引き継いでいない。design.md は大半がプローズのため画面が「見出しだけ」に見える。(2) Tasks ビューが非タスク内容 (正常な `#`/`##` 見出し・前文) を `coverGaps` で `otherBlocks(RawBlock)` に落とし、`RawBlockView` が amber 点線＝「構造化失敗フォールバック」装飾で描画するため、正常な見出しがパース失敗のように見える謎の黄色い帯になっていた。正典スケルトンの `TasksTab` は構造化タスク行のみを表示し amber 帯を出さない。
...
#### 5. 検知すべき工程で検知できなかった理由
`parseDesign` の情報無欠落不変則 (構造化 position ＋ raw ブロックの連結＝元文書全体 `[0, source.length)` を隙間なく被覆) を検証する単体テストが存在せず、そもそも `DesignDoc` に本文を運ぶ経路 (`coverGaps`) 自体が無かった。`parseTasks` / `parseRequirements` は `coverGaps`＋不変則テストを備えるのに、`parseDesign` だけ非対称に欠落していた。横並びの不変則テストがあれば「design だけ被覆していない」を即検知できた。amber 帯の UX 問題は意匠判断であり manual-verification (目視) でしか気付けないが、その確認も省略された。
...
#### 8. 根本要因詳細
正典スケルトン `SpecViewer.DesignTab` は TOC＋Traceability 表＋`<Md>{raw.design}>` で本文を全文描画していた (コメントにも「残りの design 本文は raw markdown フォールバックで全文表示 — 情報無欠落」と明記) が、本番の構造化ビューア (sdd-review-ui 4.2 design / 4.3 tasks) はこの「raw 本文全文描画」を引き継がず、`DesignDoc` 契約に本文フィールドを設けなかった。情報無欠落不変則を requirements/tasks では `coverGaps` で満たしていたが design では非対称に抜け、参照元 (スケルトン) が描画していた情報という文脈が失われた。加えて `RawBlockView` の amber「失敗フォールバック」装飾が `coverGaps` の正常コンテンツ (見出し・前文) にも一律適用される設計になっており、装飾の意味 (=構造化失敗) と適用条件 (=正常な文書チャンク) がズレていた。

#### 9. 同件調査
該当なし (既存 #0001-#0003 は evm-studio 由来の別アプリ。`design-error` × `context-loss` の組合せは初出)

#### 10. 次回からの対応策
- 構造化パーサー (design / tasks / requirements) すべてに情報無欠落不変則 (構造化＋raw の position 被覆＝`[0, source.length)`) の単体テストを横並びで用意し、特定パーサーだけ被覆が抜ける非対称を防ぐ
- 正典 (skeleton / mockup) が存在する UI 再実装では、「正典が描画している情報 (本文全文等) を本番でも必ず描画する」ことをレビュー観点・受入基準に含める
- `RawBlock` の「構造化失敗フォールバック」装飾は真の構造化失敗時のみに限定し、`coverGaps` が回収する正常コンテンツ (見出し・前文) は通常描画する。装飾の意味と適用条件を一致させる
