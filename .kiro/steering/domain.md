...
- `status`: プロジェクトの状態。有効値は active / paused / draft / archived（デフォルト active）
- `code`: プロジェクトコード（nullable text、社内識別子など任意のコード）
...
- `role`: 役割の参考プリセット（PM, Lead Eng, Engineer, Designer, QA, BA, Security, Analyst）。nullable text で自由入力可
- `initials`: 表示用イニシャル（nullable text、1-4文字）。未指定時は generateInitials で `name` から自動補完
...
| `schedule.meta.project_status` | `Project.status` | 有効値: active / paused / draft / archived、デフォルト active |
| `schedule.meta.project_code` | `Project.code` | nullable text |
| `staffing.members[].role` | `Member.role` | nullable text |
| `staffing.members[].initials` | `Member.initials` | nullable text、1-4文字、未指定時は generateInitials で自動補完 |
