...
ユニット `requirements-spec-drafted` の後（要件定義が作成完了・レビュー待ち）。要件定義の作業者は Claude、レビュー担当は太郎（指名済み）。設計・タスクは見積合意済みで未着手。**レビュー作業ノードは計画段階で既に立てられ見積合意済み**（agreed・未着手＝ready；ユニット `review-work-estimated` の後）。
...
- 出来高 EV%（達成率）：**27%**（EV_abs 3 ÷ 合意済み有効葉の見積合計 3+5+2+1=11。**葉基底**＝MODEL v18。レビュー作業ノード1人日は計画段階で既に分母に算入済み＝ユニット review-work-estimated）
- 見積カバレッジ（P2・葉基底）：**100%**（合意済み有効葉 4 / 既知の有効葉 4）
- 実行カバレッジ（R-S8・葉基底＝カウント比）：**0%**（`implementing` の合意済み有効葉 0 / 4）
- 人間レビュー待ちキュー：**1 件**（要件定義＝`implemented`・レビュー担当 太郎。レビュー作業ノードは ready＝未着手でキュー外）
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%">
  ...
  <tr>
    ...
    <td style="padding:6px 10px;border:1px solid #cbd5e1;color:#94a3b8">—</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1;color:#94a3b8">—</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1">出来高 EV% <b>27%</b>・実行 <b>0%</b>・見積カバレッジ <b>100%</b></td>
  </tr>
  <tr>
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%;margin-top:10px">
  ...
  <tr>
    ...
    <td style="padding:6px 10px;border:1px solid #cbd5e1;color:#94a3b8">—</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1;color:#94a3b8">—</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1">出来高 EV% <b>27%</b>（3/11・変化なし＝レビュー工数は計画段階で既に分母に算入済み。§7）・実行 <b>25%</b>・見積カバレッジ <b>100%</b></td>
  </tr>
  <tr>
...
<small>※ <b>2つの出来高の動きが分離して見える</b>のが本ユニットの主眼：差し戻しで<b>要件定義の出来高は 3→0</b>（再作業が必要という正直な信号・MODEL P5 非単調・部分残しをしない二値）。同時に<b>レビュー作業の出来高は 0→1</b>（太郎が実際にレビューした作業ぶん）。要件定義の作業者は差し戻し後 Claude のまま（再作業も Claude）。<b>EV% は Before 27%→During 27%→After 9%</b> と動く：レビュー工数1人日は計画段階で既に分母に算入済みゆえ During で希釈は起きず（Before も既に 3/11＝27%）、27→9 は要件定義の差し戻し（3→0）と レビュー作業の完了（0→1）の差し引き（純 EV_abs 3→1）。「安定」ではなく、それぞれ正直な信号。<b>「要件定義の出来高が 0 でも作業は無駄でない」</b>のは (a) レビュー作業の EV +1、(b) 費やした実コスト AC の残存（§4-3）、(c) 再作業はベース再利用で安く早い（次ユニット）で読める。<b>レビュー担当（reviewer）の指名属性</b>（太郎）は出来高・平準化を一切動かさない；動くのは<b>レビュー作業ノード</b>と<b>差し戻された要件定義</b>。参照スライス（`SpecValueSurface.tsx`）の担当列・レビュー担当列・レビュー作業ノード行は未描画＝定義済み要件への目標。</small>
...
| **spec-value 画面 / schedule-time 画面** | レビュー作業が「レビュー中→レビュー完了」で出来高 +1、要件定義が「レビュー待ち→再作業中」へ後退し出来高 0、玉が太郎→AI へ、EV% が 27%→27%→9% と動く（§4） |
...
- **計画段階前倒しへの同期（v2・agreed 後の改訂）:** 旧版はレビュー作業ノードの誕生・依存辺・見積合意（旧 e070–e072）を「太郎がレビューに入る時点」に置いていたが、ユニット `review-work-estimated` の agreed（計画段階で前倒し見積・合意するのが正規経路）を承け、本ユニットからその誕生・見積イベントを除き、レビュー作業ノードを**前提（既存・見積合意済み・ready）**へ移した。これに伴い **Before の出来高 EV% を 30%→27% に是正**（レビュー工数1人日が計画段階で既に分母に算入されるため。EV_abs 3 / 見積総和 3+5+2+1=11）。During の旧「30→27 希釈」説明は計画段階へ移ったため削除し、During は 27% で変化なし（After 9% は旧版で既にレビューノードを木に含めて算出済みゆえ不変）。出所＝`review-work-estimated` §7・本会話ログ（2026-06-26）。**波及（別タスク）**：同じ前倒しは `requirements-spec-drafted`・`assign-spec-provisional` など estimate-spec-agreed 以降の各ユニットの Before/木にもレビューノードを要するため、それらの同期は follow-up。
...
- **2つの出来高の分離が主眼:** 差し戻しで要件定義の出来高は 3→0、レビュー作業の出来高は 0→1。EV% は Before 27%→During 27%→After 9% と動く（レビュー工数は計画段階で既に分母算入済みゆえ During で希釈は起きず、27→9 は要件定義の差し戻し 3→0 とレビュー作業完了 0→1 の差し引き＝純 EV_abs 3→1）。レビュー工数をモデル化しないと見えるのは要件定義の出来高が 0 に落ちたことだけで「レビューは進んだか／何が起きたか」が失われる（MODEL §7#18(b) の UC を解消）。「要件定義 0 でも作業は無駄でない」は (a) レビュー作業 EV、(b) AC 残存、(c) 再作業の安さ（次ユニット）で読める。
