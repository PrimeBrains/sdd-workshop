...
- 人間レビュー待ちキュー：**2 件（導出）**（要件定義＝implemented と レビュー作業ノード＝implemented の両葉。actor 非依存導出＝MODEL §7#18(b)(viii)。一次/二次の区別の**提示**は提示層の責務——導出母集合からは消えない。参照実装のインボックス「受入判断する」区画には 2 項目とも映る）
- 玉（次の手番）：人間（太郎が承認すべき）
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%">
  ...
  <tr>
    <td style="padding:6px 10px;border:1px solid #cbd5e1">レビュー作業（要件定義）＝implemented <span style="color:#64748b">（一次/二次の区別の提示は提示層の責務・導出からは消えない・§7#18(b)(viii)）</span></td>
...
### 4-4. decision インボックス（横断）— 承認で「受入判断する」の項目が消える（承認・着手の事象そのものは項目にならない）
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%">
  ...
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1;width:34%">見積に合意する</td><td style="padding:6px 10px;border:1px solid #cbd5e1;color:#64748b">— なし（全葉合意済み） —</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">担当を割り当てる</td><td style="padding:6px 10px;border:1px solid #cbd5e1">担当割当が必要: タスク・レビュー作業（設計／タスク）の 3 件（合意済み・未割当の可視ギャップ。設計は着手時の割当で消えた）</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1;font-weight:600">受入判断する</td><td style="padding:6px 10px;border:1px solid #cbd5e1;color:#64748b">— なし。<b>Before では「受入判断が必要: 要件定義／レビュー作業（要件定義）」の 2 項目が出ていた</b>（§2 の人間レビュー待ち導出 2 件の映し）。太郎の承認（受入）で条件が消滅し、After では空 —</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">警告に対処する</td><td style="padding:6px 10px;border:1px solid #cbd5e1;color:#64748b">— なし（差し戻しリスク P5 は要件定義が accepted に到達したため消滅） —</td></tr>
</table>

<small>※ 受入区画は人間レビュー待ち（actor 非依存導出）の**同一導出の別 read＋受入操作への入口**であり、承認（受入）そのもの・設計着手の**事象**は判断項目にならない（フェーズ進行は通常の手番）。項目は条件が真の間だけ存在し、行為の追記でのみ消える——承認がまさにその行為（姉妹ユニット requirements-spec-drafted の 2026-07-04 裁定と同じ4区画構造・DECISIONS-CATALOG D-69）。</small>
...
- **WHEN** 要件定義が承認（受入）されたとき、**システムは** インボックスの「受入判断する」区画から当該項目を消さ**なければならない**（項目は条件の消滅でのみ消える——承認・差し戻しがその行為）。
- **WHEN** 要件承認または設計着手が起きたとき、**システムは** その事象そのものを新たな判断項目としてインボックスに出**してはならない**（フェーズ進行は通常の手番。判断項目は「いま判断が要る」条件が真の間だけ存在する）。
...
- **フェーズ進行の事象は判断項目にならないが、検収待ちの条件は「受入判断する」区画に映る（2026-07-04 裁定への同期・§4-4/§6 の改訂）。** 旧版は「承認・着手はインボックスに出ない」とだけ描いていたが、issue #12 がインボックスを判断種別の4区画（見積合意・担当割当・受入・警告）へ再構成し、検収待ち（人間レビュー待ち導出）が「受入判断する」区画に映るようになった（姉妹ユニット requirements-spec-drafted / returned / re-returned の 2026-07-04 実画面裁定＝実装が正、と同じ構造。DECISIONS-CATALOG D-69）。本ユニットへの帰結: **Before では受入区画に 2 項目（要件定義・レビュー作業）が出ており、太郎の承認がその条件を消滅させて After で空になる**——「承認の事象そのものは項目にならない」「5コミット判断は不変（承認は品質確認）」は従来どおり。あわせて §2/§4-2 の「提示層は畳んで隠してよい／1件に底打ちしてよい」という非正典文言を、正典 §7#18(b)(viii)「一次/二次の区別の提示は提示層の責務（導出母集合からは消えない）」へ同期した。本改訂で status を `agreed`→`in-review` に降格（再批准待ち）。
