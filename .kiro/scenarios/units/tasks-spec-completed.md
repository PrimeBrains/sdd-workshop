...
### 4-5. decision インボックス（横断）— 検収待ちは「受入判断する」区画に映り承認で消える／誕生した未見積の実装は「見積に合意する」区画に現れる
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%">
  ...
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1;width:34%;font-weight:600">見積に合意する</td><td style="padding:6px 10px;border:1px solid #cbd5e1"><b>終端断面（実装誕生後）: 「見積合意が必要: 実装-1／実装-2（提案中）」</b>——誕生した未見積の実装ノードに見積合意のコミット判断が要る、という条件の正直な映し（次ユニット estimate-impl-agreed で合意されると消える）。それ以前の断面はなし（全葉合意済み）</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">担当を割り当てる</td><td style="padding:6px 10px;border:1px solid #cbd5e1">中間断面で未割当の合意済み葉（レビュー作業（タスク）＝太郎の着手前）が可視ギャップとして出る。終端は全合意葉が割当済みで空（誕生した実装ノードは未見積のため、割当区画ではなく見積合意の区画に出る）</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1;font-weight:600">受入判断する</td><td style="padding:6px 10px;border:1px solid #cbd5e1"><b>中間断面（タスク＝レビュー待ち）: 「受入判断が必要: タスク（完了・検収待ち）」</b>（レビュー完了後はレビュー作業（タスク）も並ぶ）。**承認（受入）で条件が消滅**し、終端では空</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">警告に対処する</td><td style="padding:6px 10px;border:1px solid #cbd5e1;color:#64748b">— 全断面なし（差し戻しなし） —</td></tr>
</table>

<small>※ 2026-07-04 裁定（姉妹ユニット requirements-spec-drafted の実画面裁定と同一構造）への同期: 検収待ち（人間レビュー待ち導出）は「受入判断する」区画に**同一導出の別 read**として映る。タスク承認・実装ノード誕生（§2.6 帰結の構造操作）の**事象そのもの**は判断項目にならず、5コミット判断は不変（DECISIONS-CATALOG D-69）。ただし**誕生した未見積ノードは「見積合意が必要」という条件を新たに真にする**ため、見積合意の区画に項目が現れる——これは事象の表示ではなく条件の映しであり、正典のコミット判断①（見積合意）そのもの。E2E は終端のタスク不出現を回帰固定し、実装誕生の絶対非表示の主張は成立しないことを注記済み（本改訂でその「仮 agreed」を解消）。</small>
...
- **WHEN** タスクが作成完了になったとき、**システムは** それを横断の decision インボックスの**コミット判断の区画（見積合意・担当割当）**に出**してはならない**（承認はコミット判断ではない）。
- **WHEN** タスクが作成完了になったとき、**システムは** それをインボックスの**「受入判断する」区画**に検収待ちとして現し、承認（受入）まで保た**なければならない**（この区画は人間レビュー待ちの映し＋受入操作の入口であり、人間のコミット判断の数を増やさない）。
...
- **WHEN** タスクが承認（受入）されたとき、**システムは** インボックスの「受入判断する」区画から当該項目を消さ**なければならない**（項目は条件の消滅でのみ消える——承認・差し戻しがその行為）。
- **WHEN** タスク承認や実装ノード誕生が起きたとき、**システムは** その事象そのものを新たな判断項目としてインボックスに出**してはならない**（フェーズ進行・システム内部の構造操作は通常の手番）。
- **WHEN** 実装ノードが未見積のまま誕生したとき、**システムは** それをインボックスの「見積に合意する」区画に見積合意が必要な項目として現さ**なければならない**（見積合意は人間のコミット判断であり、その条件が真になったことの正直な映し。合意で消える）。
...
- **フェーズ進行・実装誕生の事象は判断項目にならないが、条件はインボックスの各区画に正直に映る（2026-07-04 裁定への同期・§1/§4-5/§6 の改訂）。** 旧版は「タスクのレビュー待ち・承認・実装誕生はインボックスに出ない」と描いていたが、issue #12 がインボックスを判断種別の4区画（見積合意・担当割当・受入・警告）へ再構成したことで、(a) 検収待ち（人間レビュー待ち導出）は「受入判断する」区画に映り承認で消える（姉妹ユニット requirements-spec-drafted の 2026-07-04 実画面裁定＝実装が正、と同一構造。DECISIONS-CATALOG D-69）、(b) **誕生した未見積の実装ノードは「見積に合意する」区画に「見積合意が必要」として現れる**——事象の表示ではなく「見積合意のコミット判断が要る」という条件の映しで、正典のコミット判断①そのもの（E2E 同期時に「実装誕生の絶対非表示は主張できない（仮 agreed）」と記録され、本改訂で解消）。「承認・誕生の事象そのものは項目にならない」「5コミット判断は不変」は従来どおり。**§3 の括弧書き「（横断の decision インボックスには出ない）」は人間発案の記述のため本改訂では触れていない**——再批准時の人間改訂に委ねる（§1/§4-5/§6 は同期済み）。本改訂で status を `agreed`→`in-review` に降格（再批准待ち）。
