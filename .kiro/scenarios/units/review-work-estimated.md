...
- **requirements-spec-returned.md との関係（同期済み）:** 旧 requirements-spec-returned は「太郎がレビューに入る時点」でレビュー作業ノードを立てる経路（旧 e070–e072）を含んでいたが、本ユニットの agreed を承け **2026-06-26 に同期済み**——レビュー作業ノードの誕生・依存辺・見積合意を**前提（計画段階で実施済み＝本ユニットの e040–e046）**へ移し、requirements-spec-returned は計画済みレビュー作業ノードの**実行**（着手→完了→差し戻し・e073–e075）のみを扱う形に改めた。これに伴い同ユニットの Before 出来高 EV% は 30%→**24%** へ是正された（本ユニットが計画段階で 3 レビューノード〔1・1・0.5〕を作るため、下流の木は 6 葉・見積総和 12.5；当初 1 葉・分母 11 とした暫定は本カスケードで是正）。requirements-spec-drafted も同じ 6 葉・分母 12.5 で同期済み（After EV% 30→24）。assign-spec-provisional は EV%/分母を持たないが、レビュー工数の可視化に揃えて **6 葉（フェーズ 3＋レビュー作業 3）に暫定割当を描く形へ 2026-06-27 に同期済み**（本ユニットの後＝レビュー作業ノード追加後の断面として位置づけ・未割当バックログ 6→0。旧整理の「位置独立ゆえ変更不要」を撤回）。cancel-task-midway は実装タスクのキャンセルが主題でフェーズレビューと無関係ゆえ対象外。
