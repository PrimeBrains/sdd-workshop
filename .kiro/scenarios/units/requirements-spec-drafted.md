...
- この「レビュー待ち＝玉が人間」は**人間のレビュー待ち一覧に現れる**こと（要件: moira-surface-schedule Req13）。横断の decision インボックスでは、コミット判断の区画ではなく**「受入判断する」区画**に検収待ちとして映る（2026-07-04 裁定同期・§7）。
...
### 4-5. decision インボックス（横断）— 検収待ちは「受入判断する」区画に出る
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%">
  ...
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">担当を割り当てる</td><td style="padding:6px 10px;border:1px solid #cbd5e1">担当割当が必要: 設計・タスク・レビュー作業（要件定義／設計／タスク）の 5 件（合意済み・未割当の可視ギャップ＝未割当バックログの映し。本ユニットの主題外＝前提木に由来）</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1;font-weight:600">受入判断する</td><td style="padding:6px 10px;border:1px solid #cbd5e1"><b>受入判断が必要: F/req（完了・検収待ち）</b> — 受入または差し戻しで消滅</td></tr>
  <tr><td style="padding:6px 10px;border:1px solid #cbd5e1">警告に対処する</td><td style="padding:6px 10px;border:1px solid #cbd5e1;color:#64748b">— なし —</td></tr>
</table>

<small>※ 2026-07-04 裁定（実画面提示・ユーザー）: 従来の「ここには出ない」記述は issue #12（受入判断の集約）の実装と矛盾が顕在化し、**実装側が正**と裁定された。「受入判断する」区画は既存の人間レビュー待ち（actor 非依存の導出）の**同一導出の別 read＋受入操作への入口**であり、**5コミット判断は不変**（承認は品質確認のまま・MODEL §2.1）。コミット判断の区画（見積合意・担当割当）に承認が出ないことは従来どおり。E2E は裁定後の挙動を回帰固定済み。</small>
...
- **WHEN** 要件定義が作成完了になったとき、**システムは** それを横断の decision インボックスの**コミット判断の区画（見積合意・担当割当）**に出**してはならない**（承認はコミット判断ではない）。
- **WHEN** 要件定義が作成完了になったとき、**システムは** それをインボックスの**「受入判断する」区画**に検収待ちとして現さ**なければならない**（この区画は人間レビュー待ちの映し＋受入操作の入口であり、人間のコミット判断の数を増やさない）。
...
- **decision インボックスは判断種別4区画——検収待ちは「受入判断する」区画に出る（2026-07-04 裁定・従来決定の改訂）:** 本ユニットは当初「成果物の承認はインボックスに出ない（5判断のうち4つにも判断要警告にも非該当）」と確定していた（[UI-ARCHITECTURE.md](../../../moira/UI-ARCHITECTURE.md) §3・MODEL §2.1）。その後 issue #12（ユーザー起票）がインボックスを判断種別の4区画（見積合意・担当割当・受入・警告）へ再構成し「受入判断する」区画を新設したことで矛盾が顕在化し、E2E 同期（2026-07-04・1f53887）時に実画面提示のうえ**ユーザーが実装側を正と裁定**した。受入区画は既存の人間レビュー待ち導出（actor 非依存・`humanReviewQueue`）の**同一導出の別 read＋受入操作への入口**であり、**5コミット判断は不変**（承認は品質確認のまま。DECISIONS-CATALOG D-69）。「承認がコミット判断の区画（見積合意・担当割当）に出ない」ことは従来どおり不変。なお**レビュー担当の指名**は §2.1#2 割当に属する人間のコミット判断だが、これは割当 write の文脈で扱い、本論点とは別。**§3 の括弧書き「（横断の decision インボックスには出ない）」は人間発案の記述のため本改訂では触れていない**——裁定後の実態（受入区画に映る）と齟齬があるため、再批准時の人間改訂に委ねる（§1/§4-5/§6 は同期済み）。本改訂で §1/§4-5・§6 当該節を書き換え、status を `agreed`→`in-review` に降格（再批准待ち）。
