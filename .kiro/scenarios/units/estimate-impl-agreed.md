...
採用表現は **spec-value 中心**（前段ユニット `estimate-spec-proposed` §7 のガント不採用決定を継承）。実装レビューは「ノードとして立てる」を選んだ場合を主に描く（畳む場合の数値は §4 末尾「畳む分岐」に併記）。
...
<table style="border-collapse:collapse;font-family:system-ui,sans-serif;font-size:13px;width:100%;margin-top:10px">
  ...
  <tr>
    ...
    <td style="padding:6px 10px;border:1px solid #cbd5e1">—</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1"><span style="background:#e2e8f0;border-radius:4px;padding:1px 6px">pending</span></td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1">見積もり済み <b>75%→67%</b>（6/9）<span style="color:#dc2626">⚠ レビューノードを追加発見（葉 8→9）</span><br>達成率 <b>100%</b>（提案は未算入）・<span style="color:#b45309">提案あり 3 葉</span></td>
  </tr>
  <tr>
    <td style="padding:6px 10px 6px 28px;border:1px solid #cbd5e1">└ （フェーズ6葉）</td>
    <td style="padding:6px 10px;border:1px solid #cbd5e1"><span style="background:#bbf7d0;border-radius:4px;padding:1px 6px">承認済み</span></td>
...
**データ（After・畳む分岐 — 実装レビューを軽微として畳んだ場合）**
...
| └ 実装-1 | pending | agreed | 8人日 | 8 | ✅ |
| └ 実装-2 | pending | agreed | 6人日 | 6 | ✅ |
| （実装レビュー専用ノードは作られない＝§6 EARS） | — | — | — | — | — |

見積もり済みの割合（P2・葉基底）：**100%**（合意済み有効葉 8 / 既知の有効葉 8）<br>
凍結済み合計：**26.5人日**（12.5 + 8 + 6）／達成率 EV%：**約47%**（EV_abs 12.5 / 26.5）<br>
<small>※ 畳む分岐では実装レビュー葉が立たないため既知の葉は 8 のまま（ノード化分岐の 9 と対照）。達成率はノード化分岐の 44% より高い 47% になる——レビュー工数が分母に算入されないぶん見かけ上高く出るだけで、実体は同じ。レビュー工数は実費（cost）として計上されるが、その計上はレビュー作業が実際に発生する実装フェーズ以降であり、本ユニットの After 時点（着手前）ではまだ cost は発生していない。畳んだレビューの cost は被レビューノードに帰属する（MODEL §7#18(b)(iv)）。</small>
...
<small>注：**e050 は本ユニットの前提を生む起点イベント**（tasks 完了の §2.6 帰結であり、本ユニットの precondition＝「実装は誕生済み・未見積」を成立させる）。本ユニットが扱うのは以降（e051〜）。e050 により既知の葉が 6→8 に増え、見積もり済みの割合が 100%→75% に下がる（未見積子は I1 総和から除外＝moira-core Req11.2）。画面の「見積なし」は内部表現では `latestEstimate=null`（`moira/backend/src/types.ts` の `ProjectedNode`）に対応する——`estimateState` 列挙（`proposed`/`agreed`）とは独立の軸で、見積値が未だ載っていない状態を指す。</small>
...
- **est(impl) の見積活動自体は畳む（活動ノード化しない）:** 「実装ノードを見積もる」という活動（est(impl)）そのものは本ユニットでは独立ノードに立てず、見積案を載せる decompose（e051）一段として畳む。R-E2b は「見積活動が相応に重ければノード化・軽微なら畳んで cost 計上＝人間の P0 判断」だが、本シナリオでは見積活動を軽微として畳む既定を採る——EV を持つ見積ノードが立つのは被見積対象（実装ノード・実装レビューノード）であって、見積活動自体ではない。出所＝MODEL R-E1b/R-E2b・`review-work-estimated` と同型。
- **実装ノード誕生（100%→75%）は本ユニットに切り出さず起点注記とする（FORK 解決・ユーザー裁定）:** tasks 完了の §2.6 帰結である実装ノードの誕生（見積もり済み 100%→75% への低下）を、独立したシナリオ・ユニットには**切り出さない**。本ユニットは誕生後（75% 起点）を扱い、誕生イベント e050 は本ユニットの precondition を生む起点として §5 に注記するに留める。誕生イベントの flow 上の最終帰属（tasks-spec-completed の postcondition に含めるか）は、後続の `kiro-scenario-flow` による合成時に裁定する。出所＝ユーザー裁定（FORK §7(d)＝切り出さない）。
- **「成果物が無ければ AI が分解を提案する」は本ユニットの対象外＝境界宣言で確定（FORK 解決・ユーザー裁定）:** 作業分解の成果物が無い場合に AI が分解を提案するパスは、§3 が宣言する通り**本ユニットの対象外**（分解そのものを作るのはタスク分解フェーズの責務で、深さの確定は人間の P0 判断）。これは閉じた境界宣言であり未決論点ではない。別ユニットの起票は将来必要に応じて判断する（本ユニットの agreed 条件には影響しない）。出所＝ユーザー裁定（FORK §7(c)＝境界宣言として確定）・MODEL §2.1#4。
- **検証ループの確定記録:** トレース ID は敵対者ループで是正済み（reviewer 6.4/6.6 除去・I1 除外 11.1/11.2 追加・surface 7.1→7.3／3.1・6.2 追加）で、doc-fact-checker により全 ID の実在・適合を CONFIRMED。例示見積値（実装 8／6・実装レビュー 2 人日）は golden fixture `tiny-project.ts`（impl-1=8・impl-2=6）に倣った仮値で、実値は実装・運用が決める。出所＝doc-fact-checker 照合・`tiny-project.ts`。
