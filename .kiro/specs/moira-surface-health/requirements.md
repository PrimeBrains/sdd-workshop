...
### Requirement 11: 管理者の人間対応待ち（ボトルネック）提示と AI 稼働指標の位置づけ（R-S8/P4・surface-health Req10.6・UI-ARCH §3/§5）

**Objective:** 健全性を読む管理者として、AI 稼働の活発さ（実行カバレッジ・エージェント作業キュー）だけでプロジェクトの停滞有無を判断せず、いま人間の判断を待っている作業の件数と最も古い滞留年齢を一目で読みたい。それにより、エージェント加速下で制約が人間側の関門へ移ったとき、ボトルネックの所在（合意・承認の滞留）を取り違えずに把握できる。

> **境界の明示（MODEL 変更なし・既存導出の提示）:** 本要件が提示する「人間対応待ち（合意待ち＋承認待ち＋最古年齢）」は `moira-schedule` が導出する人間ゲート待ち read（`moira-schedule` Req15）を**読むだけ**で再計算しない（UI-ARCH §6 二系統計算の禁止）。役割（管理者/開発者）は同一 R-S2 導出への actor 種別プリセット（初期＝管理者；Req10 AC6・UI-ARCH §5）であって物理分割でない。**スコープは2ゲート限定**で完全なボトルネック信号ではない（未割当バックログ・警告は別表示）。execCov（R-S8／Req4）の式・要件は不変——本要件は execCov を「進捗でなく処理能力側の仕掛中量」と位置づけ直すのみ（R-S8 が既に『in-progress VOLUME, not progress』と規定する範囲の提示判断＝P0 提示の自由）。受け入れシナリオ `.kiro/scenarios/units/manager-spots-review-bottleneck.md`（§3 人間批准済）が駆動材料。

#### Acceptance Criteria

1. The system shall present the human-gate backlog — the count of work awaiting a human decision, unioned over agreement-waiting (`proposed`) and review-waiting (`implemented`), together with its oldest age — as derived by `moira-schedule` (Req15), without re-computing the backlog set or the age in this surface.
   - 和訳: システムは人間対応待ち——合意待ち（`proposed`）と承認待ち（`implemented`）を和集合した人間判断待ちの件数と、その最も古い年齢——を `moira-schedule`（Req15）が導出する値として提示し、本サーフェスで待ち集合や年齢を再計算してはならない。
2. While AI activity (execution coverage / the agent work queue) is active, the system shall not present AI activity alone as indicating whether the project is stalled, presenting execution coverage as processing-capacity-side in-progress volume rather than as the bottleneck signal (R-S8: in-progress volume, not progress — the formula is unchanged; this is a presentation positioning, P0).
   - 和訳: AI 稼働（実行カバレッジ／エージェント作業キュー）が活発な間、システムは AI 稼働だけでプロジェクトの停滞有無を表してはならず、実行カバレッジを処理能力側の仕掛中量として提示し、ボトルネック信号として提示してはならない（R-S8: 仕掛中の量であって進捗でない。式は不変＝提示の位置づけ・P0）。
3. The system shall not sum the human-gate backlog with EV%, execution coverage, or any earned-value figure, presenting it as a distinct dimension (a count of human-pending work with age), an R-S4/R-S6-isomorphic de-rate read.
   - 和訳: システムは人間対応待ちを、EV%・実行カバレッジ・その他の出来高量と算術和してはならず、別次元（年齢つきの人間待ち件数）として提示しなければならない（R-S4/R-S6 同型の de-rate read）。
4. The system shall present the human-gate backlog scoped to the two pipeline gates only and shall not present it as the complete bottleneck signal — the unassigned backlog (R-U9, hosted by the schedule surface) and the health warnings (at-risk P5, stale-slot R-S7) are separate human-pending signals — and shall present a leaf merely parked in a gate as still counted (its genuine blockage read via those other signals), never dropping it from the visible accounting (P0).
