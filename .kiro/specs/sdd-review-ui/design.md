...
- npm 依存（すべて MIT・ローカルバンドル）: react 19 / react-dom / react-router 7（library mode）/ @tanstack/react-query 5 / react-markdown + remark-gfm / mermaid（`securityLevel: 'strict'` 固定）/ tailwindcss 4 / vite / vitest + @testing-library/react + msw（dev）/ @playwright/test（dev）
- 禁止: markdown 由来 HTML の `dangerouslySetInnerHTML` 注入（管理された唯一の例外 = MermaidBlock が注入する mermaid 生成 SVG。Security Considerations 参照）、`any` 型、外部 CDN・外部 API、`evm-studio/` からの import、書込エンドポイント呼び出し、データベース
...
- Steering compliance: structure.md の「データフェッチは TanStack Query を介し hooks に封じ込め」「コンポーネント PascalCase / フック use プレフィックス」「Single Source of Truth（API ベース URL・ポートは 1 箇所）」、tech.md の `dangerouslySetInnerHTML` 禁止を踏襲（mermaid 生成 SVG の管理された例外のみ Security Considerations で規定）。Phase 2 制約（roadmap.md）が EVM Studio スタック記述に優先する箇所: tRPC ではなく sdd-core の REST + 契約型を消費する
...
| Diagram | mermaid | `mermaid` フェンスコードブロックの図描画 | `securityLevel: 'strict'` で初期化。生成 SVG の注入は MermaidBlock のみ |
...
│       │   ├── MermaidBlock.tsx    # mermaid 図描画（securityLevel: 'strict'。失敗時 raw + 警告）
...
| `/specs/:feature/:document` | SpecDocumentPage（document = brief / requirements / design / tasks / research） | 1.4, 3.9。ハッシュ `#<anchor>` でジャンプ位置・フォーカス対象復元 |
| `/specs/:feature/compare?left=...&right=...` | ComparePage | 4.1, 4.2 |
| `/specs/:feature/matrix` | MatrixPage | 5.1。ハッシュでフォーカス行復元（3.7） |
...
ナビゲーション状態（画面・スペック・ドキュメント・フォーカス対象）はすべて上記の URL パス・クエリ・ハッシュに符号化する（3.7）。ジャンプを含むすべての遷移はブラウザ履歴 push で行い、戻る / 進むでフォーカス・スクロール対象を含む状態が復元される（3.8）。
...
アンカー解決に失敗した場合（design 要素名と見出しの照合外れ等）は黙って無視しない: ジャンプ先が design 対応先であれば、design ビューの**構造化トレーサビリティ行**（該当要件の行）へ遷移してフォールバックする（デッドクリックなし → 3.10）。それ以外はドキュメント先頭へ遷移し「対象位置を特定できなかった」notice を表示する。
...
| 2.8 | mermaid ブロックの図描画 | MermaidBlock, RawBlockView | mermaid（`securityLevel: 'strict'`） | — |
| 2.9 | mermaid 失敗時の raw コード + 警告表示 | MermaidBlock | フォールバック描画 | — |
...
| 3.7 | ナビゲーション状態の URL 符号化 | Router, JumpNavigation | ルート表, URL ハッシュ | 相互リンクジャンプ |
| 3.8 | ブラウザ戻る / 進むでの状態復元 | Router, JumpNavigation | ブラウザ履歴 push | 相互リンクジャンプ |
| 3.9 | ディープリンクの復元とフォーカススクロール | SpecDocumentPage, JumpNavigation | URL ハッシュ | — |
| 3.10 | design アンカー未解決時のトレーサビリティ行フォールバック | JumpNavigation, RefChip, DesignView | anchors, `resolved: false` | 相互リンクジャンプ |
...
| AppShell + Router | Shell | レイアウト・ルートレジストリ・予約名前空間 | 1.4, 3.7, 3.8, 7.2 | react-router (P0) | State |
...
| JumpNavigation（anchors + useJump + jumpHistory） | Navigation | アンカー規約・スクロール・ハイライト・履歴・URL 符号化 | 3.3, 3.4, 3.7, 3.9, 3.10, 5.4 | — | Service, State |
| DocBlockList + RawBlockView + MermaidBlock + MarkdownDoc | Markdown | DocBlock ディスパッチ + 安全 raw 描画 + mermaid 図描画 | 2.5, 2.6, 2.7, 2.8, 2.9, 8.2 | react-markdown (P0), mermaid (P0) | Service |
...
| RefChip + CounterpartPopover + JumpBackBar | Feature: crosslink | 参照チップ・対応先提示・broken / cross-spec 表示 | 3.1, 3.2, 3.5, 3.6, 3.10, 4.4 | TraceIndex (P0), JumpNavigation (P0) | Service |
...
| Requirements | 1.4, 3.7, 3.8, 7.2, 8.1 |
...
- ナビゲーション状態（画面・スペック・ドキュメント・フォーカス対象）の遷移はすべてブラウザ履歴 push として行う（3.7）。ブラウザの戻る / 進むは URL の復元 = ナビゲーション状態の復元（フォーカス・スクロール対象を含む）として機能する（3.8）
...
- State model: URL が唯一のビュー位置の真実（画面 / spec / document / 比較ペイン / フォーカス対象アンカー）。リロード・共有リンクで同一ビュー + フォーカス位置を復元（1.4, 3.7, 3.9）
- Persistence & consistency: 永続化なし。再取得時もルートが変わらないため選択は保持される（7.2）。ブラウザ履歴のエントリ単位 = ナビゲーション状態 1 つ（3.8）
...
| Field | Detail |
|-------|--------|
| Intent | アンカー ID 規約・ジャンプ実行・履歴・URL フォーカス符号化の横断基盤 |
| Requirements | 3.3, 3.4, 3.7, 3.9, 3.10, 5.4 |
...
- `useJump`: アンカーへ `scrollIntoView({ block: "center" })` + 2 秒の一時ハイライトクラス付与。アンカー不在時は `{ resolved: false }` を返し、呼び出し側がフォールバックを実行する（黙って無視しない）: design 対応先は design ビューの構造化トレーサビリティ行へ遷移（3.10）、その他はドキュメント先頭 + notice
- `jumpTo` は遷移先（ルート + アンカー）をブラウザ履歴 push で URL に反映する（フォーカス対象の URL 符号化 → 3.7）。ルート読み込み時は URL ハッシュからフォーカス対象を復元してスクロールする（リロード・共有ディープリンク → 3.9）
- `jumpHistory.ts`: ジャンプ前の位置（ルート + アンカー）をスタックに push。`back()` で pop して復元（3.4）。ブラウザ履歴と独立の「出自へ戻る」UI 内履歴（JumpBackBar に表示）であり、ブラウザの戻る / 進む（3.8）と併存する
...
#### DocBlockList + RawBlockView + MermaidBlock + MarkdownDoc

| Field | Detail |
|-------|--------|
| Intent | 情報無欠落原則のクライアント側実装の唯一の所有者 |
| Requirements | 2.5, 2.6, 2.7, 2.8, 2.9, 8.2 |
...
- `MermaidBlock`: RawBlockView / MarkdownDoc のコードブロック描画で言語指定 `mermaid` を検出したとき、mermaid ライブラリ（`securityLevel: 'strict'` で初期化 — 図中の raw HTML / script を無効化）で図として描画する（2.8）。mermaid が生成した SVG の DOM 注入は本コンポーネントのみに許される管理された例外（markdown 由来 HTML の注入は引き続き禁止）。描画失敗（構文エラー等）時は生コード全文を目に見える警告とともに表示し、黙って欠落させない（2.9 — 情報無欠落原則）
...
- External: mermaid — `mermaid` コードブロックの図描画 (P0)
...
| Requirements | 3.1, 3.2, 3.5, 3.6, 3.10, 4.4 |
...
- クリック時: TraceIndex で対応先を引き、CounterpartPopover に design / task 別に一覧表示 → 選択で `jumpTo`（3.1, 3.2 — マトリクスを経由しない直接ジャンプ）。requirement ノード上のチップ（例 AC の自 ID）も同じポップオーバーで両方向を提示する。design 対応先のアンカーが解決できない場合は design ビューの構造化トレーサビリティ行へフォールバック遷移する（3.10 — デッドクリックなし）
...
| アンカー解決失敗 | ジャンプ先 DOM 不在（名称照合外れ等） | design 対応先は構造化トレーサビリティ行へフォールバック遷移（3.10）。その他はドキュメント先頭へ遷移 + 「対象位置を特定できなかった」notice（黙って無視しない） |
...
7. `MermaidBlock`: 正常な mermaid ソースが svg 要素として描画され、構文エラーのソースが生コード全文 + 警告表示にフォールバックすること（svg なし・コード全文一致を厳密に検証）（2.8, 2.9）
...
- **XSS が最大の脅威面**: `.kiro/` の markdown は信頼できない入力として扱う。**markdown 由来 HTML の `dangerouslySetInnerHTML` 注入は禁止**（react-markdown は raw HTML 非描画設定 = rehype-raw を導入しない）。管理された唯一の例外は、mermaid ライブラリが `securityLevel: 'strict'`（図中の raw HTML / script を無効化）で生成した SVG を **MermaidBlock コンポーネントのみ**が注入すること。markdown 文字列やその派生 HTML をそれ以外の経路で注入することは引き続き全面禁止。`urlTransform` で `javascript:` 等の危険スキームと外部オリジン URL を無効化。React デフォルトエスケープに依存する構造化ビューも同様
