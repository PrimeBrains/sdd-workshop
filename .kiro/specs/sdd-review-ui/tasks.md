...
- [x] 10.2 読み取り専用・ローカル完結の検証テストを実装する
...
- 10.2: `readonly-local.spec.ts`（E2E ネットワークログ: 外部オリジン0件・サーバー非GET0件、SSE は GET 許容、ログ非空＋GET /api/specs・/api/events 観測の事前アサートで偽 pass 防止、SpecActionSlot 空＝書込 UI なし）+ `check-dist-no-external-urls.ts`（dist の取得参照 src/href/url() は外部ゼロ容赦なし、文字列定数は許可リスト方式）。`verify:local` = build→scan。【教訓】`/g` 正規表現を `.test()` と `matchAll()` で共有すると `lastIndex` が持ち越され各ファイル先頭 URL を取りこぼす — `freshUrlRegex()` で都度生成。`--self-test` で先頭 URL 検出を回帰ガード（レビュー初回リジェクト→是正済み）
