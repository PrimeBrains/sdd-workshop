...
- [ ] 3.2 ドキュメント表示ページとルーティング結線を実装する
  ...
  - 完了条件: document パラメータごとに対応するビューが描画され、リロードで同一ドキュメント + アンカー位置が復元される
  - _Requirements: 1.4_
...
- [ ] 5.2 アンカー規約とジャンプ実行を実装する
  ...
  - アンカー解決失敗時はドキュメント先頭へ遷移し「対象位置を特定できなかった」notice を表示する
...
- [ ] 5.3 参照チップと対応先ポップオーバーを実装する
  ...
  - 完了条件: AC チップ → ポップオーバー → design 対応先選択で design ルートへ遷移しハイライトされる結合テストと、broken-link チップがクリックしても遷移しないテストが通る
