...
- capacity heatmap は decision 面で host も描画もせず、c 起因の判断（R-T3 過負荷・R-S7 の c 起因分）は capacity config 面（`moira-capacity` skill）への deep-link でルーティングする（heatmap の host・READ 表示・c 宣言の write はすべて capacity config 面が所有）。
