...
- **隣接期待値**: evm-engine は本スペックが定義する Task 型・DB テーブル構造に依存する。インポート後に Task.estimate_days・planned_start・planned_end が正しく登録されていることを前提に PV 計算を行う（planned_start/planned_end は tasks.yaml または schedule.yaml の assignments[] から設定される）。progress-tracking は本スペックが登録した初回 ProgressSnapshot を起点として進捗を更新する。
...
6. The EVM Studio shall define a `progress_snapshots` table with columns: `id`, `task_id` (FK to tasks), `snapshot_date` (TEXT NOT NULL), `progress_pct` (REAL NOT NULL, 0〜100), `pv_days` (REAL NOT NULL, default 0), `ev_days` (REAL NOT NULL, default 0), `ac_days` (REAL NOT NULL, default 0), `created_at`.
   *EVM Studio は `progress_snapshots` テーブルを定義する（カラム: `id`、`task_id`（tasks への FK）、`snapshot_date`（TEXT NOT NULL）、`progress_pct`（REAL NOT NULL、0〜100）、`pv_days`（REAL NOT NULL、デフォルト 0）、`ev_days`（REAL NOT NULL、デフォルト 0）、`ac_days`（REAL NOT NULL、デフォルト 0）、`created_at`）。リスケ後も過去スナップショット時点の PV/EV が正確に参照できるよう、記録時点の計算済み値を保存する。*
...
11. When a task entry in tasks.yaml has a null or absent `estimate_days` field, the EVM Studio API shall treat the value as 0 without returning an error. When a task entry has a null or absent `planned_start` or `planned_end` field, the EVM Studio API shall store null for those fields without returning an error.
    *tasks.yaml のタスクエントリで `estimate_days` が null または欠落している場合、API はエラーを返さず 0 として扱う。`planned_start` または `planned_end` が null または欠落している場合、API はエラーを返さず null として保存する。（wbs-* スキルが生成するスケジュール分離型 YAML では planned 日付は tasks.yaml に含まれず schedule.yaml 側の assignments に記録される）*
12. When schedule.yaml contains an `assignments[]` array where each entry has a `task_id` (external_id format), `planned_start`, and `planned_end`, the EVM Studio API shall update the corresponding task's `planned_start` and `planned_end` with those values. If an assignment references a `task_id` that was not found in the imported tasks, the EVM Studio API shall silently skip that assignment.
    *schedule.yaml が `task_id`（external_id 形式）・`planned_start`・`planned_end` を持つ `assignments[]` 配列を含む場合、API は対応するタスクの `planned_start` および `planned_end` を割り当て値で更新する。インポートされたタスクに存在しない `task_id` を参照する割り当ては無視する。*
13. When creating a ProgressSnapshot during import and the task's `planned_start` or `planned_end` is null, the EVM Studio API shall set `pv_days` to 0.
    *インポート中に ProgressSnapshot を作成する際、タスクの `planned_start` または `planned_end` が null の場合、API は `pv_days` を 0 に設定する。*
