...
`moira-surface-schedule` は Moira 正典モデル `moira/MODEL.md`(v16, 凍結) を本番アーキテクチャへ落とす **CQRS 分解の Wave3 read サーフェス**であり、`moira/UI-ARCHITECTURE.md` が R-S2 に予約する三ダッシュボードの一つ `schedule-time`（スケジュール・時間）を担う。UI-ARCHITECTURE §3 のとおり、これは **全 actor 共通の母 view** であり、A1 上の射影（projection）であって独立した一級実体ではない。
...
3. The system shall display agent work as schedulable spans between human touchpoints, visually distinct from human work, without drawing an over-allocation marker on agent rows (agents are not leveled; A5/R-U11).
   - 和訳: システムはエージェント作業を、人間接点間の実行可能スパンとして人間作業と視覚的に区別して表示し、エージェント行に過負荷マーカーを描いてはならない（エージェントは平準化されない；A5/R-U11）。
...
### Requirement 14: 視点 actor「自分」フィルタ（自分のレビュー待ち／自分が reviewer の項目）（P4・§7#18(f)・UI-ARCH §2/§5）

**Objective:** 利用者（いま見ている開発者）として、レビュー待ち一覧やキューを「自分（例：太郎）のレビュー待ち／自分が reviewer の項目」だけに絞り込みたい。それにより、種別（全員/人間/エージェント）フィルタでは表せない「自分の分」を発見できる。これは被割当者（assignee）とも actor 種別フィルタ（Req8）とも別の、**視点 actor（いま誰が見ているか）**を要する。

> **Reference-implementation deviation（視点 actor 不在で「自分」は現状無効）:** フォワード本番の参照実装 `moira/frontend/src/surfaces/schedule/ScheduleTimeSurface.tsx` は actor 種別フィルタ（全員／人間（レビュー/作業）／エージェント）に加えて「自分」選択肢を持つが、**視点 actor が backend に無いため無効化されている**（同ファイルの「自分」選択肢は `title="視点 actor が backend に無いためブロッカー…valid-time/視点 actor 拡張で解禁。"` で disabled）。視点 actor は被割当者（assignee）でも reviewer（per-node 指名）でもなく、**提示層/セッション入力**であり MODEL は保持しない（§7#18(f)：「自分のレビュー」は視点 actor で、reviewer〔per-node 指名〕とは別概念。MODEL §2.1 提示の自由・UI-ARCH §2「役割はモデル外」）。本 Req14 は視点 actor の導入＋「自分」絞り込みを**新規要件**として起票し、現状（backend 不在で無効）を明記する。

#### Acceptance Criteria

1. The system shall provide a viewpoint-actor "self" filter that narrows the review-waiting list / queues to the items the current viewing developer reviews — "my review-waiting" and "items where I am the designated reviewer" — by matching the per-node `reviewer` (and, for review-waiting, lifecycle=`implemented`) against the viewpoint actor.
   - 和訳: システムは視点 actor「自分」フィルタを提供し、レビュー待ち一覧／キューを、いま見ている開発者がレビューする項目——「自分のレビュー待ち」「自分が指名 reviewer の項目」——へ絞り込まなければならない。これは per-node の `reviewer`（およびレビュー待ちでは lifecycle=`implemented`）を視点 actor と突き合わせて行う。
2. The system shall treat the viewpoint actor as a presentation-layer / session input that the MODEL does not hold (§7#18(f); UI-ARCH §2 "role is outside the model"), distinct from both the assignee and the per-node reviewer designation, and shall apply it as a filter over the same single derivation — never as a physically separate computation per viewpoint (the screen-layer "two truths" anti-pattern is forbidden).
   - 和訳: システムは視点 actor を、MODEL が保持しない**提示層/セッション入力**（§7#18(f)；UI-ARCH §2「役割はモデル外」）として、被割当者とも per-node の reviewer 指名とも区別して扱い、同一の単一導出に対するフィルタとして適用しなければならない——視点ごとに物理的に別計算してはならない（画面層の「二つの真実」反パターンは禁止）。
3. The system shall keep the underlying human review queue derivation actor-independent and unchanged regardless of the viewpoint filter (the queue membership does not change; only the displayed subset is narrowed), consistent with `moira-schedule` Req14 AC2 — the "self" narrowing is a presentation filter, not a re-derivation.
   - 和訳: システムは、視点フィルタに依らず、基盤の人間レビュー待ちキュー導出を actor 非依存・不変に保たなければならない（キューの母集合は変わらず、絞り込むのは表示部分集合のみ）。これは `moira-schedule` Req14 AC2 と整合する——「自分」絞り込みは提示フィルタであって再導出ではない。
4. While the upstream backend does not supply a viewpoint actor (the current reference-implementation state), the system shall present the "self" option as disabled (a known gap), and shall not silently substitute the assignee or a role kind for the viewpoint actor.
   - 和訳: 上流 backend が視点 actor を供給しない間（現行の参照実装状態）、システムは「自分」選択肢を無効（既知ギャップ）として提示し、視点 actor の代わりに被割当者や役割種別を暗黙に代用してはならない。
